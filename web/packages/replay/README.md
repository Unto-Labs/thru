...
| `view` (block), `minConsensus` (block/tx), `returnEvents` (tx) | Block/tx factories | Mirror Thru RPC query flags so callers can trade fidelity for throughput. |
...
- Protobuf types are not generated in this package; they are imported from `@thru/sdk/proto`.
...
With this README plus the in-repo sources, you should have everything you need to operate, extend, or debug the replay pipeline with confidence.
