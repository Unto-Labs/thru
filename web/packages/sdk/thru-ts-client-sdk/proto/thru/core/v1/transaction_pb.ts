...
export type TransactionExecutionResult = Message<"thru.core.v1.TransactionExecutionResult"> & {
   ...
   * payer, as of this transaction's execution. Populated ONLY when vm_error is
   * TRANSACTION_VM_ERROR_NONCE_TOO_LOW or TRANSACTION_VM_ERROR_NONCE_TOO_HIGH;
   * absent otherwise. A nonce reject does not advance the nonce, so retrying
   * with this value is correct.
   *
   * This is not necessarily the account's live nonce by the time you read it:
   * another transaction from the same fee payer may have landed since. Treat it
   * as a correction to retry against, not a reservation. For a fee payer
   * restored by a creation state proof it is 0, and no account exists yet.
   *
   * @generated from field: optional uint64 fee_payer_expected_nonce = 14;
   */
  feePayerExpectedNonce?: bigint | undefined;
};
...
export enum TransactionVmError {
  ...
  /**
   * TN_RUNTIME_TXN_ERR_FEE_PAYER_ACTIVATION_REQUIRES_STATE_UNIT
   *
   * @generated from enum value: TRANSACTION_VM_ERROR_FEE_PAYER_ACTIVATION_REQUIRES_STATE_UNIT = -497;
   */
  TRANSACTION_VM_ERROR_FEE_PAYER_ACTIVATION_REQUIRES_STATE_UNIT = -497,

  /**
   * TN_RUNTIME_TXN_ERR_BLOCK_STATE_BUDGET_EXCEEDED
   *
   * @generated from enum value: TRANSACTION_VM_ERROR_BLOCK_STATE_BUDGET_EXCEEDED = -496;
   */
  TRANSACTION_VM_ERROR_BLOCK_STATE_BUDGET_EXCEEDED = -496,
