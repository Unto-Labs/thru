...
use crate::abi::types::{
  ArrayType, EnumType, FloatingPointType, IntegralType, PrimitiveType, SizeDiscriminatedUnionType, StructType, TypeDef, TypeKind, TypeRefType,
  UnionType,
};
...
impl DependencyGraph {
  ...
  pub fn add_dependency(&mut self, dep: Dependency) {
    ...
    self.adjacency_list.entry(dep.from.clone()).or_insert_with(Vec::new).push(dep.to.clone());
  ...
  pub fn detect_cycles(&self) -> Vec<CyclePath> {
    ...
    for node in &self.nodes {
      if !visited.contains(node) {
        self.dfs_cycle_detection(node, &mut visited, &mut rec_stack, &mut path, &mut cycles);
      }
    }
    ...
    if let Some(neighbors) = self.adjacency_list.get(node) {
      for neighbor in neighbors {
        if !visited.contains(neighbor) {
          ...
          cycles.push(CyclePath { cycle: complete_cycle, dependencies: cycle_deps });
        }
      }
    }
  ...
  pub fn topological_sort(&self) -> Option<Vec<String>> {
    ...
    for dep in &self.edges {
      *in_degree.entry(dep.from.clone()).or_insert(0) += 1;
      ...
      reverse_adjacency.entry(dep.to.clone()).or_insert_with(Vec::new).push(dep.from.clone());
    }
...
impl DependencyAnalyzer {
  pub fn new() -> Self {
    Self { graph: DependencyGraph::new(), current_type_name: None, field_path: Vec::new() }
  }
  ...
  fn analyze_type_kind(&mut self, type_kind: &TypeKind) {
    match type_kind {
      ...
      TypeKind::SizeDiscriminatedUnion(size_disc_union) => self.analyze_size_discriminated_union(size_disc_union),
  ...
  fn analyze_type_ref(&mut self, type_ref: &TypeRefType) {
    if let Some(current_type) = &self.current_type_name {
      let context = if self.field_path.is_empty() { "direct type reference".to_string() } else { format!("field: {}", self.field_path.join(".")) };
  ...
  fn analyze_expression(&mut self, expr: &ExprKind, dep_kind: DependencyKind) {
    match expr {
      ...
      ExprKind::Sizeof(sizeof_expr) => {
        ...
        if let Some(current_type) = &self.current_type_name {
          let context = format!("sizeof expression in field: {}", self.field_path.join("."));
          self.graph.add_dependency(Dependency {
      ...
      ExprKind::Alignof(alignof_expr) => {
        ...
        if let Some(current_type) = &self.current_type_name {
          let context = format!("alignof expression in field: {}", self.field_path.join("."));
          self.graph.add_dependency(Dependency {
            from: current_type.clone(),
  ...
  fn analyze_field_reference(&mut self, field_ref: &FieldRefExpr, dep_kind: DependencyKind) {
    if let Some(current_type) = &self.current_type_name {
      let context = format!("field reference '{}' in field: {}", field_ref.path.join("."), self.field_path.join("."));
      ...
      self.graph.add_dependency(Dependency { from: current_type.clone(), to: target_field, kind: dep_kind, context });
    }
  }
  ...
  fn validate_type_internal_duplicates(&mut self, typedef: &TypeDef) {
    match &typedef.kind {
      ...
      TypeKind::SizeDiscriminatedUnion(size_disc_union) => {
        self.validate_size_discriminated_union_variant_duplicates(&typedef.name, size_disc_union);
      }
    ...
    for field in &struct_type.fields {
      if !field_names.insert(field.name.clone()) {
        self.graph.add_validation_error(ValidationError {
          ...
          reason: format!("Field name '{}' appears multiple times in struct '{}'", field.name, type_name),
        });
      }
    }
  }
    ...
    for variant in &union_type.variants {
      if !variant_names.insert(variant.name.clone()) {
        self.graph.add_validation_error(ValidationError {
          ...
          reason: format!("Variant name '{}' appears multiple times in union '{}'", variant.name, type_name),
        });
      }
    }
  }
    ...
    for variant in &enum_type.variants {
      ...
      if !variant_names.insert(variant.name.clone()) {
        self.graph.add_validation_error(ValidationError {
          ...
          violating_type: type_name.to_string(),
          duplicate_name: variant.name.clone(),
          reason: format!("Variant name '{}' appears multiple times in enum '{}'", variant.name, type_name),
        });
      }
      ...
      if !tag_values.insert(variant.tag_value) {
        self.graph.add_validation_error(ValidationError {
          ...
          reason: format!("Tag value '{}' is used by multiple variants in enum '{}'", variant.tag_value, type_name),
        });
      }
    }
  }

  fn validate_size_discriminated_union_variant_duplicates(&mut self, type_name: &str, size_disc_union: &SizeDiscriminatedUnionType) {
    let mut variant_names = HashSet::new();
    ...
    for variant in &size_disc_union.variants {
      ...
      if !variant_names.insert(variant.name.clone()) {
        self.graph.add_validation_error(ValidationError {
          ...
          violating_type: type_name.to_string(),
          duplicate_name: variant.name.clone(),
          reason: format!("Variant name '{}' appears multiple times in size-discriminated union '{}'", variant.name, type_name),
        });
      }
    }
  }
  ...
  fn validate_type_layout_constraints(&mut self, typedef: &TypeDef, all_typedefs: &[TypeDef]) {
    match &typedef.kind {
      ...
      TypeKind::SizeDiscriminatedUnion(size_disc_union) => {
        self.validate_size_discriminated_union_constraints(&typedef.name, size_disc_union, all_typedefs);
      }
      _ => {} // Other types don't have layout-affecting expressions
    }
  }

  fn validate_enum_tag_constraints(&mut self, enum_name: &str, enum_type: &EnumType, all_typedefs: &[TypeDef]) {
    ...
    for field_ref in field_refs {
      ...
      if let Some(violation) = self.check_enum_tag_layout_cycle(enum_name, &field_ref, all_typedefs) {
        self.graph.add_layout_violation(violation);
      }
    }
  }

  fn validate_array_size_constraints(&mut self, array_name: &str, array_type: &ArrayType, all_typedefs: &[TypeDef]) {
    // Check if element type has non-constant size
    if let Some(violation) = self.check_array_element_type_size(array_name, array_type, all_typedefs) {
      self.graph.add_layout_violation(violation);
    }
    ...
    for field_ref in field_refs {
      if let Some(violation) = self.check_array_size_layout_cycle(array_name, &field_ref, all_typedefs) {
        self.graph.add_layout_violation(violation);
      }
    }
  }

  fn validate_struct_field_constraints(&mut self, struct_name: &str, struct_type: &StructType, all_typedefs: &[TypeDef]) {
    ...
    for (field_index, field) in struct_type.fields.iter().enumerate() {
      if let Some(violation) = self.check_field_layout_dependency(struct_name, field_index, field, all_typedefs) {
        self.graph.add_layout_violation(violation);
      }
    }
  }

  fn check_enum_tag_layout_cycle(&self, enum_name: &str, field_ref: &str, all_typedefs: &[TypeDef]) -> Option<LayoutConstraintViolation> {
    ...
    let enum_size_depends_on_tag = if let Some(enum_typedef) = all_typedefs.iter().find(|td| td.name == enum_name) {
    ...
    let is_same_struct_reference = self.is_enum_referencing_same_container_struct(enum_name, &ref_type, all_typedefs);

    if is_same_struct_reference {
      // Check if this is a forward reference (enum field comes before referenced field in the struct)
      if let Some(forward_ref_violation) = self.check_enum_forward_reference_in_same_struct(enum_name, &ref_type, &ref_field, all_typedefs) {
    ...
    if let Some(dependency_chain) = self.find_layout_dependency_chain(&ref_type, enum_name, all_typedefs) {
  ...
  fn check_array_size_layout_cycle(&self, array_name: &str, field_ref: &str, all_typedefs: &[TypeDef]) -> Option<LayoutConstraintViolation> {
    let (ref_type, ref_field) = self.parse_field_reference(field_ref);

    if let Some(dependency_chain) = self.find_layout_dependency_chain(&ref_type, array_name, all_typedefs) {
      return Some(LayoutConstraintViolation {
        ...
        if ref_type == struct_name {
          if let Some(struct_def) = all_typedefs.iter().find(|td| td.name == struct_name) {
            if let TypeKind::Struct(struct_type) = &struct_def.kind {
              ...
              if let Some(ref_field_index) = struct_type.fields.iter().position(|f| f.name == ref_field) {
                if ref_field_index > field_index {
                  ...
                  let creates_layout_dependency = self.type_size_depends_on_field_refs(&field_type_def, all_typedefs)
                    || self.type_needs_field_ref_for_layout(&field_type_def, &field_ref);

                  // Special case: Allow enums that don't affect the referenced field's offset
                  let is_acceptable_enum_reference = if let TypeKind::Enum(_) = &field_type_def.kind {
                    !self.does_enum_size_affect_field_offset(field_index, ref_field_index, &field_type_def, all_typedefs)
                  } else {
                    false
                  };
                  ...
                  if creates_layout_dependency && !is_acceptable_enum_reference {
                    return Some(LayoutConstraintViolation {
                      ...
                      violating_expression: format!("field '{}' type contains reference to field '{}'", field.name, field_ref),
                      dependency_chain: vec![struct_name.to_string(), field.name.clone(), ref_field.clone()],
                      reason: format!(
    ...
    match &typedef.kind {
      ...
      TypeKind::Struct(struct_type) => {
        ...
        for field in &struct_type.fields {
          ...
          if let Some(field_typedef) = self.find_typedef_for_type(&field.field_type, all_typedefs) {
            if self.type_size_depends_on_field_refs(field_typedef, all_typedefs) {
              return true;
            }
          } else {
            // Handle inline types (not TypeRef)
            if self.type_size_depends_on_field_refs_recursive(&field.field_type, all_typedefs) {
              return true;
            }
          }
        }
      ...
      TypeKind::Union(union_type) => {
        ...
        for variant in &union_type.variants {
          // Check both named types (via TypeRef) and inline types
          if let Some(variant_typedef) = self.find_typedef_for_type(&variant.variant_type, all_typedefs) {
            if self.type_size_depends_on_field_refs(variant_typedef, all_typedefs) {
              return true;
            }
          } else {
            // Handle inline types (not TypeRef)
            if self.type_size_depends_on_field_refs_recursive(&variant.variant_type, all_typedefs) {
              return true;
            }
          }
        }
  ...
  fn type_needs_field_ref_for_layout(&self, typedef: &TypeDef, field_ref: &str) -> bool {
    ...
    match &typedef.kind {
      ...
      TypeKind::Union(union_type) => {
        // Check recursively for nested types
        for variant in &union_type.variants {
          if let Some(variant_typedef) = self.find_typedef_for_type(&variant.variant_type, &[]) {
  ...
  fn find_layout_dependency_chain(&self, from_type: &str, to_type: &str, _all_typedefs: &[TypeDef]) -> Option<Vec<String>> {
  ...
  fn parse_field_reference(&self, field_ref: &str) -> (String, String) {
    ...
    if let Some(pos) = field_ref.find("::") {
      ...
      let current_type = self.current_type_name.as_ref().unwrap_or(&"unknown".to_string()).clone();
  ...
  fn find_typedef_for_type<'a>(&self, type_kind: &TypeKind, all_typedefs: &'a [TypeDef]) -> Option<&'a TypeDef> {
    match type_kind {
  ...
  fn collect_field_references_in_type_kind(&self, type_kind: &TypeKind, field_refs: &mut Vec<String>) {
    match type_kind {
      TypeKind::Array(array_type) => {
  ...
  fn is_enum_referencing_same_container_struct(&self, enum_name: &str, ref_type: &str, all_typedefs: &[TypeDef]) -> bool {
        ...
        if let (Some(enum_idx), Some(ref_idx)) = (enum_field_index, ref_field_index) {
          if enum_idx < ref_idx {
            ...
            if let Some(enum_typedef) = all_typedefs.iter().find(|td| td.name == enum_name) {
              ...
              let enum_affects_tag_offset = self.does_enum_size_affect_field_offset(enum_idx, ref_idx, enum_typedef, all_typedefs);
            ...
            return Some(LayoutConstraintViolation {
              violating_type: enum_name.to_string(),
              violating_expression: format!("enum tag references field: {}.{}", struct_name, ref_field),
              dependency_chain: vec![struct_name.to_string(), enum_name.to_string(), ref_field.to_string()],
              reason: format!(
  ...
  fn check_array_element_type_size(&self, array_name: &str, array_type: &ArrayType, all_typedefs: &[TypeDef]) -> Option<LayoutConstraintViolation> {
  ...
  fn type_size_depends_on_field_refs_recursive(&self, type_kind: &TypeKind, all_typedefs: &[TypeDef]) -> bool {
    match type_kind {
      ...
      TypeKind::Array(array_type) => {
        ...
        self.type_size_depends_on_field_refs_recursive(&array_type.element_type, all_typedefs)
      }
      TypeKind::SizeDiscriminatedUnion(_) => {
        // Size-discriminated unions always have variable size by definition
        true
  ...
  fn validate_size_discriminated_union_constraints(
    ...
    for typedef in all_typedefs {
      self.check_size_discriminated_union_usage_in_type(union_name, &typedef.name, &typedef.kind, all_typedefs);
    }

    // Also validate that all variants have different sizes (otherwise it should be a regular Union)
    self.validate_size_discriminated_union_variant_sizes(union_name, size_disc_union, all_typedefs);
  }
  ...
  fn check_size_discriminated_union_usage_in_type(
    ...
    match type_kind {
      TypeKind::Struct(struct_type) => {
        ...
        let has_size_disc_union = self.struct_contains_size_discriminated_union(struct_type, union_name, all_typedefs);
        if has_size_disc_union {
          // Check if the union is the sole size-affecting factor
          if !self.is_sole_size_affecting_factor_in_struct(struct_type, union_name, all_typedefs) {
      ...
      TypeKind::Array(array_type) => {
        ...
        if self.type_contains_size_discriminated_union(&array_type.element_type, union_name, all_typedefs) {
          self.graph.add_layout_violation(LayoutConstraintViolation {
            violating_type: containing_type.to_string(),
      ...
      TypeKind::Union(_) | TypeKind::Enum(_) => {
        ...
        if self.type_contains_size_discriminated_union_recursive(type_kind, union_name, all_typedefs) {
          self.graph.add_layout_violation(LayoutConstraintViolation {
            violating_type: containing_type.to_string(),
            violating_expression: format!("contains size-discriminated union: {}", union_name),
            dependency_chain: vec![containing_type.to_string(), union_name.to_string()],
  ...
  fn struct_contains_size_discriminated_union(&self, struct_type: &StructType, union_name: &str, all_typedefs: &[TypeDef]) -> bool {
    for field in &struct_type.fields {
      if self.type_contains_size_discriminated_union(&field.field_type, union_name, all_typedefs) {
        return true;
      }
    }
    false
  ...
  fn is_sole_size_affecting_factor_in_struct(&self, struct_type: &StructType, union_name: &str, all_typedefs: &[TypeDef]) -> bool {
  ...
  fn type_contains_size_discriminated_union(&self, type_kind: &TypeKind, union_name: &str, all_typedefs: &[TypeDef]) -> bool {
    match type_kind {
      TypeKind::TypeRef(type_ref) => {
        if type_ref.name == union_name {
          ...
          if let Some(typedef) = all_typedefs.iter().find(|td| td.name == type_ref.name) {
            self.type_contains_size_discriminated_union_recursive(&typedef.kind, union_name, all_typedefs)
          } else {
            false
          }
        }
      }
      _ => self.type_contains_size_discriminated_union_recursive(type_kind, union_name, all_typedefs),
    }
  }

  fn type_contains_size_discriminated_union_recursive(&self, type_kind: &TypeKind, union_name: &str, all_typedefs: &[TypeDef]) -> bool {
    match type_kind {
      TypeKind::Struct(struct_type) => self.struct_contains_size_discriminated_union(struct_type, union_name, all_typedefs),
      TypeKind::Union(union_type) => {
        for variant in &union_type.variants {
          if self.type_contains_size_discriminated_union(&variant.variant_type, union_name, all_typedefs) {
            return true;
          }
        }
        false
      ...
      TypeKind::Array(array_type) => self.type_contains_size_discriminated_union(&array_type.element_type, union_name, all_typedefs),
      TypeKind::SizeDiscriminatedUnion(_) => {
      ...
      TypeKind::TypeRef(_type_ref) => self.type_contains_size_discriminated_union(type_kind, union_name, all_typedefs),
  ...
  fn type_has_variable_size(&self, type_kind: &TypeKind, all_typedefs: &[TypeDef]) -> bool {
    match type_kind {
      ...
      TypeKind::Array(array_type) => {
        ...
        !array_type.size.is_constant() || self.type_has_variable_size(&array_type.element_type, all_typedefs)
      }
    }
  }
