...
impl AbiFile {
  ...
  pub fn new(metadata: AbiMetadata) -> Self {
    Self { abi: metadata, types: Vec::new() }
  }
...
impl ImportResolver {
  ...
  pub fn load_file_with_imports(&mut self, file_path: &Path, verbose: bool) -> anyhow::Result<()> {
    ...
    if self.loaded_files.contains(&canonical_path) {
      if verbose {
        println!("    [~] Skipping already loaded file: {}", file_path.display());
      }
      return Ok(());
    }
    ...
    let type_names: Vec<String> = abi_file.get_types().iter().map(|t| t.name.clone()).collect();
