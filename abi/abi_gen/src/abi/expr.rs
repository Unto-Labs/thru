...
impl FieldRefExpr {
  ...
  pub fn to_c_field_access(&self) -> String {
    let tag_path: Vec<String> =
      self.path.iter().flat_map(|s| s.split('/')).map(|s| s.trim_start_matches("..")).filter(|s| !s.is_empty()).map(|s| s.to_string()).collect();

    if tag_path.is_empty() { "tag".to_string() } else { tag_path.join(".") }
  }
}
...
impl ExprKind {
  ...
  pub fn try_evaluate_constant(&self) -> Option<u64> {
    match self {
      ...
      ExprKind::Eq(_) | ExprKind::Ne(_) | ExprKind::Lt(_) | ExprKind::Gt(_) | ExprKind::Le(_) | ExprKind::Ge(_) => None, // Comparison results are booleans
  ...
  pub fn to_debug_string(&self) -> String {
    match self {
      ...
      ExprKind::Add(expr) => {
        format!("Add({} + {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Sub(expr) => {
        format!("Sub({} - {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Mul(expr) => {
        format!("Mul({} * {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Div(expr) => {
        format!("Div({} / {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Mod(expr) => {
        format!("Mod({} % {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Pow(expr) => {
        format!("Pow({} ** {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::BitAnd(expr) => {
        format!("BitAnd({} & {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::BitOr(expr) => {
        format!("BitOr({} | {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::BitXor(expr) => {
        format!("BitXor({} ^ {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::LeftShift(expr) => {
        format!("LeftShift({} << {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::RightShift(expr) => {
        format!("RightShift({} >> {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::BitNot(expr) => {
      ...
      ExprKind::Eq(expr) => {
        format!("Eq({} == {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Ne(expr) => {
        format!("Ne({} != {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Lt(expr) => {
        format!("Lt({} < {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Gt(expr) => {
        format!("Gt({} > {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Le(expr) => {
        format!("Le({} <= {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Ge(expr) => {
        format!("Ge({} >= {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::And(expr) => {
        format!("And({} && {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Or(expr) => {
        format!("Or({} || {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
      ExprKind::Xor(expr) => {
        format!("Xor({} ^^ {})", expr.left.to_debug_string(), expr.right.to_debug_string())
      }
    }
  }
}
