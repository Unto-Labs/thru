...
use super::helpers::{format_type_to_rust, generate_nested_field_access, primitive_to_rust_type};
...
fn emit_size_fn_struct(resolved_type: &ResolvedType) -> String {
    ...
    if let ResolvedTypeKind::Struct { fields, .. } = &resolved_type.kind {
        /* Find the first FAM */
        let first_fam_idx = fields
            .iter()
            .position(|f| matches!(f.field_type.size, Size::Variable(_)));

        if let Some(fam_idx) = first_fam_idx {
            /* Start with offset of first FAM */
            write!(
                output,
                "        let mut offset: usize = std::mem::offset_of!({}, {});\n",
                type_name, fields[fam_idx].name
            )
            .unwrap();
                ...
                match &field.field_type.kind {
                    ...
                    ResolvedTypeKind::Array { element_type, .. } => {
                        if let Size::Variable(var_refs) = &field.field_type.size {
                            ...
                            for (field_ref_path, prim_type) in
                                var_refs.values().flat_map(|m| m.iter())
                            {
                    ...
                    ResolvedTypeKind::TypeRef { .. } | ResolvedTypeKind::Struct { .. } => {
                        if let Size::Variable(var_refs) = &field.field_type.size {
                            ...
                            write!(
                                output,
                                "            let field_ptr = std::ptr::addr_of!(self.{});\n",
                                field.name
                            )
                            .unwrap();
