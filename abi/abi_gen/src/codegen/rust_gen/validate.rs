...
fn emit_validate_fn_struct(resolved_type: &ResolvedType) -> String {
  ...
  write!(output, "pub fn {}_validate(buffer: &[u8]) -> Result<(), &'static str> {{\n", type_name).unwrap();

  /* Check minimum size */
  write!(output, "    /* Check minimum buffer size */\n").unwrap();
  write!(output, "    if buffer.len() < std::mem::size_of::<{}_t>() {{\n", type_name).unwrap();
  write!(output, "        return Err(\"buffer too small for struct header\");\n").unwrap();
  write!(output, "    }}\n\n").unwrap();

  if let ResolvedTypeKind::Struct { fields, .. } = &resolved_type.kind {
    let has_fams = fields.iter().any(|f| matches!(f.field_type.size, Size::Variable(_)));

    if has_fams {
      write!(output, "    /* Validate FAM sizes */\n").unwrap();
      write!(output, "    let ptr = buffer.as_ptr() as *const {}_t;\n", type_name).unwrap();
      write!(output, "    let mut offset: u64 = std::mem::size_of::<{}_t>() as u64;\n", type_name).unwrap();
          ...
          match &field.field_type.kind {
            ResolvedTypeKind::Array { element_type, .. } => {
              if let Some(var_map) = var_refs.values().next() {
                if let Some((first_ref, _)) = var_map.iter().next() {
                  ...
                  /* Check for overflow */
                  write!(output, "    if {} < 0 {{\n", size_var).unwrap();
                  write!(output, "        return Err(\"FAM size cannot be negative\");\n").unwrap();
                  write!(output, "    }}\n").unwrap();

                  match &element_type.size {
                    Size::Const(elem_size) => {
                      write!(output, "    let fam_size = ({} as u64).checked_mul({})\n", size_var, elem_size).unwrap();
                      write!(output, "        .ok_or(\"FAM size overflow\")?;\n").unwrap();
                      write!(output, "    offset = offset.checked_add(fam_size)\n").unwrap();
                      write!(output, "        .ok_or(\"offset overflow\")?;\n").unwrap();
                    }
                    Size::Variable(_) => {
                      /* Multi-dimensional FAM */
                      write!(output, "    let elem_size = {}_footprint({});\n",
                        format_type_to_rust(element_type), size_var).unwrap();
            ...
            _ => {
              ...
              if field_refs.is_empty() {
                write!(output, "    let field_size = {}_footprint();\n",
      ...
      write!(output, "        return Err(\"buffer too small for FAM data\");\n").unwrap();
      write!(output, "    }}\n\n").unwrap();
    }
      ...
      match &field.field_type.kind {
        ResolvedTypeKind::Struct { .. } | ResolvedTypeKind::TypeRef { .. } => {
          if matches!(field.field_type.size, Size::Const(_)) {
            ...
            write!(output, "    {{\n").unwrap();
            write!(output, "        let field_offset = std::mem::offset_of!({}_t, {});\n",
              type_name, field.name).unwrap();
            write!(output, "        let field_slice = &buffer[field_offset..];\n").unwrap();
            write!(output, "        {}_validate(field_slice)?;\n",
              format_type_to_rust(&field.field_type)).unwrap();
            write!(output, "    }}\n").unwrap();
          }
        }
...
fn emit_validate_fn_enum(resolved_type: &ResolvedType) -> String {
  ...
  if let ResolvedTypeKind::Enum { variants, tag_expression, .. } = &resolved_type.kind {
    write!(output, "pub fn {}_validate(buffer: &[u8]) -> Result<(), &'static str> {{\n", type_name).unwrap();

    /* Check minimum size */
    write!(output, "    /* Check minimum buffer size */\n").unwrap();
    write!(output, "    if buffer.len() < std::mem::size_of::<{}_t>() {{\n", type_name).unwrap();
    write!(output, "        return Err(\"buffer too small for enum\");\n").unwrap();
    write!(output, "    }}\n\n").unwrap();
    ...
    if let crate::abi::expr::ExprKind::FieldRef(field_ref) = tag_expression {
      ...
      write!(output, "        /* TODO: Proper tag reading based on path: {} */\n", tag_path).unwrap();
