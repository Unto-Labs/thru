...
use super::helpers::{
    escape_rust_keyword, format_type_to_rust, is_nested_complex_type, primitive_to_rust_type,
};
...
fn emit_init_fn_struct(resolved_type: &ResolvedType) -> String {
    ...
    write!(
        output,
        "    let mut required_size: u64 = std::mem::size_of::<{}_t>() as u64;\n",
        type_name
    )
    .unwrap();
    ...
    for info in &field_infos {
        if info.is_fam {
          ...
          info.param_name, info.param_name).unwrap();
                write!(
                    output,
                    "        .ok_or(\"arithmetic overflow calculating array size\")?;\n"
                )
                .unwrap();
            } else {
                /* FAM non-array */
          ...
          info.param_name).unwrap();
                write!(
                    output,
                    "        .ok_or(\"arithmetic overflow calculating field size\")?;\n"
                )
                .unwrap();
            }
        }
    ...
    write!(
        output,
        "    let ptr = buffer.as_mut_ptr() as *mut {}_t;\n",
        type_name
    )
    .unwrap();
    write!(output, "    unsafe {{\n").unwrap();
    ...
    for info in &field_infos {
        if !info.is_fam {
            ...
            if info.is_array {
                write!(
                    output,
                    "        std::ptr::write_unaligned(std::ptr::addr_of_mut!((*ptr).{}), *{});\n",
    ...
    if has_fams {
        ...
        write!(
            output,
            "    let mut offset: usize = std::mem::size_of::<{}_t>();\n",
            type_name
        )
        .unwrap();
        ...
        for info in &field_infos {
            if info.is_fam {
                if info.is_array {
                    ...
                    write!(
                        output,
                        "        let slice_size = {}.len() * std::mem::size_of_val(&{}[0]);\n",
                        info.param_name, info.param_name
                    )
                    .unwrap();
                    write!(
                        output,
                        "        let src_ptr = {}.as_ptr() as *const u8;\n",
                        info.param_name
                    )
                    .unwrap();
                    write!(output, "        unsafe {{\n").unwrap();
                    ...
                    write!(
                        output,
                        "        let field_size = std::mem::size_of_val({});\n",
                        info.param_name
                    )
                    .unwrap();
                    write!(
                        output,
                        "        let src_ptr = {} as *const _ as *const u8;\n",
                        info.param_name
                    )
                    .unwrap();
                    write!(output, "        unsafe {{\n").unwrap();
