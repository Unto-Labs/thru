...
pub fn format_type_to_rust(type_kind: &TypeKind, indent: usize) -> String {
  match type_kind {
    ...
    TypeKind::SizeDiscriminatedUnion(sdu_type) => emit_anonymous_size_discriminated_union(sdu_type, indent),
  }
}

/* Check if a type is a nested complex type that needs extraction */
fn is_nested_complex_type(type_kind: &TypeKind) -> bool {
  matches!(type_kind, TypeKind::Struct(_) | TypeKind::Union(_) | TypeKind::Enum(_) | TypeKind::SizeDiscriminatedUnion(_))
}
  ...
  match field_type {
    TypeKind::Struct(struct_type) => {
      ...
      for field in &struct_type.fields {
        if is_nested_complex_type(&field.field_type) {
          ...
          collect_nested_type_definitions(&nested_parent_name, &field.name, &field.field_type, parent_attributes, output);
        }
      }
    ...
    TypeKind::Enum(enum_type) => {
      ...
      for variant in &enum_type.variants {
        ...
        write!(union_content, "    pub {}: {},  // tag: {}\n", escaped_name, variant_type_str, variant.tag_value).unwrap();
      }
...
fn format_field_declaration(field_name: &str, field_type: &TypeKind, indent: usize) -> String {
  ...
  match field_type {
    TypeKind::Array(array_type) => {
      ...
      format!("{}: [{}; {}]", escaped_name, format_type_to_rust(&array_type.element_type, indent), size_expr)
    }
    _ => {
      format!("{}: {}", escaped_name, format_type_to_rust(field_type, indent))
    }
  }
}

/* Format field declaration with nested type handling for top-level structs */
fn format_field_declaration_with_nested(parent_name: &str, field_name: &str, field_type: &TypeKind, indent: usize) -> String {
  let escaped_name = escape_rust_keyword(field_name);
  match field_type {
    TypeKind::Array(array_type) => {
      let size_expr = format_expr_to_rust(&array_type.size);
...
fn emit_anonymous_struct(struct_type: &crate::abi::types::StructType, indent: usize) -> String {
  ...
  for field in &struct_type.fields {
    let field_decl = format_field_declaration(&field.name, &field.field_type, indent + INDENT_BASE);
...
fn emit_anonymous_union(union_type: &crate::abi::types::UnionType, indent: usize) -> String {
  ...
  for variant in &union_type.variants {
    ...
    let escaped_name = escape_rust_keyword(&variant.name);
    write!(output, "{}{}: {},\n", field_indent, escaped_name, variant_type_str).unwrap();
  }
...
fn emit_anonymous_enum(enum_type: &crate::abi::types::EnumType, indent: usize) -> String {
  ...
  for variant in &enum_type.variants {
    let variant_type_str = format_type_to_rust(&variant.variant_type, indent + INDENT_BASE);
    let escaped_name = escape_rust_keyword(&variant.name);
    write!(output, "{}{}: {}, /* tag: {} */\n", field_indent, escaped_name, variant_type_str, variant.tag_value).unwrap();
  }
...
fn emit_anonymous_size_discriminated_union(sdu_type: &crate::abi::types::SizeDiscriminatedUnionType, indent: usize) -> String {
  let mut output = String::new();
  write!(output, "union {{\n").unwrap();
  ...
  for variant in &sdu_type.variants {
    let variant_type_str = format_type_to_rust(&variant.variant_type, indent + INDENT_BASE);
    let escaped_name = escape_rust_keyword(&variant.name);
    write!(output, "{}{}: {}, /* expected size: {} */\n", field_indent, escaped_name, variant_type_str, variant.expected_size).unwrap();
  }
  ...
  match &type_def.kind {
    TypeKind::Struct(struct_type) => {
      ...
      for field in &struct_type.fields {
        ...
        if is_nested_complex_type(&field.field_type) {
          collect_nested_type_definitions(&type_def.name, &field.name, &field.field_type, &struct_type.container_attributes, &mut output);
        }
      }
    ...
    TypeKind::Union(union_type) => {
      ...
      for variant in &union_type.variants {
        let escaped_name = escape_rust_keyword(&variant.name);
        write!(union_content, "    pub {}: {},\n", escaped_name, format_type_to_rust(&variant.variant_type, INDENT_FIELD)).unwrap();
      }

      output.push_str(&emit_container_definition(&union_type.container_attributes, &type_def.kind, &type_def.name, &union_content));
    }
    TypeKind::Enum(enum_type) => {
      // Add enum header comment
      write!(output, "/* VARIANTS FOR: {} */\n", type_def.name.to_uppercase()).unwrap();
      ...
      for variant in &enum_type.variants {
        write!(output, "pub const ENUM_{}_{}: u64 = {};\n", type_def.name.to_uppercase(), variant.name.to_uppercase(), variant.tag_value).unwrap();
      }
      write!(output, "\n").unwrap();
    }
    TypeKind::Array(array_type) => {
      ...
      let size_expr = format_expr_to_rust(&array_type.size);
      write!(array_content, "    pub data: [{}; {}],\n", format_type_to_rust(&array_type.element_type, INDENT_FIELD), size_expr).unwrap();

      output.push_str(&emit_container_definition(&array_type.container_attributes, &type_def.kind, &type_def.name, &array_content));
    }
    TypeKind::SizeDiscriminatedUnion(sdu_type) => {
      ...
      output.push_str(&emit_container_definition(&sdu_type.container_attributes, &type_def.kind, &type_def.name, &sdu_content));
    }
    _ => {
