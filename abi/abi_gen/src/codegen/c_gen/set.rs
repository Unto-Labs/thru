...
use std::fmt::Write;
use super::helpers::{escape_c_keyword, primitive_to_c_type, format_type_to_c, format_expr_to_c, generate_nested_field_access, is_nested_complex_type};
    ...
    for variant in variants {
      ...
      match &variant_type.kind {
        ResolvedTypeKind::Primitive { .. } => {
          ...
          writeln!( output, "void {}_set_{}( {}_t * self, {} const * value ) {{", type_name, escaped_variant_name, type_name, c_type ).unwrap();
          writeln!( output, "  memcpy( &self->{}, value, sizeof( {} ) );", escaped_variant_name, c_type ).unwrap();
        }
        ResolvedTypeKind::Array { element_type, size_expression, .. } => {
          ...
          writeln!( output, "void {}_set_{}( {}_t * self, {} const * value, uint64_t len ) {{", type_name, escaped_variant_name, type_name, element_c_type ).unwrap();
          let expected_len_expr = format_expr_to_c(&size_expression, &[]);
          writeln!( output, "  assert( len == ({}) );", expected_len_expr ).unwrap();
          writeln!( output, "  memcpy( self->{0}, value, len * sizeof self->{0}[0] );", escaped_variant_name ).unwrap();
        }
        ...
        _ => {
          ...
          writeln!( output, "void {}_set_{}( {}_t * self, {} const * value ) {{", type_name, escaped_variant_name, type_name, target_name).unwrap();
          writeln!( output, "  memcpy( &self->{}, value, sizeof( {} ) );", escaped_variant_name, target_name ).unwrap();
        }
      }
      writeln!( output, "}}\n" ).unwrap();
    }
        ...
        for field in fields.iter() {
            ...
            match &field.field_type.kind {
                ResolvedTypeKind::Primitive { prim_type } => {
                    ...
                    write!(output, "void {}_set_{}( {}_t * self, {} value ) {{\n", type_name, escaped_name, type_name, field_type_str).unwrap();
                    ...
                    if after_variable_size_data {
                        ...
                        write!(output, "  memcpy( (unsigned char *)self + offset, &value, sizeof( value ) );\n").unwrap();
                    } else {
                        write!(output, "  memcpy( &self->{}, &value, sizeof( value ) );\n", escaped_name).unwrap();
                    }
                ...
                ResolvedTypeKind::TypeRef { target_name, .. } => {
                    ...
                    write!(output, "void {}_set_{}( {}_t * self, {}_t const * value ) {{\n", type_name, escaped_name, type_name, target_name).unwrap();

                    /* Perform a memcpy to copy the nested struct */
                    if after_variable_size_data {
                        output.push_str(&fam_offset_code);
                        write!(output, "  memcpy( (unsigned char *)self + offset, value, sizeof( {}_t ) );\n", target_name).unwrap();
                    } else {
                        write!(output, "  memcpy( &self->{}, value, sizeof( {}_t ) );\n", escaped_name, target_name).unwrap();
                    }
                ...
                ResolvedTypeKind::Enum { variants, .. } => {
                    ...
                    write!(output, "void const * {}_get_{}( {}_t const * self ) {{\n",
                           type_name, escaped_name, type_name).unwrap();
                    if after_variable_size_data {
                        output.push_str(&fam_offset_code);
                        write!(output, "  return (void const *)((unsigned char const *)self + offset);\n").unwrap();
                    } else {
                    ...
                    write!(output, "void {}_set_{}( {}_t * self, void const * value ) {{\n",
                           type_name, escaped_name, type_name).unwrap();
                    write!(output, "  /* Copy variant data - size determined by tag value */\n").unwrap();
                    write!(output, "  uint64_t variant_size = {}_size( self ) - sizeof( {}_t );\n",
                           type_name, type_name).unwrap();
                    if after_variable_size_data {
                        output.push_str(&fam_offset_code);
                        write!(output, "  memcpy( (unsigned char *)self + offset, value, variant_size );\n").unwrap();
                    } else {
                ...
                _ => {
                    /* Skip other field types - no setters for arrays, nested structs/unions */
                }
            }
...
pub fn emit_set_fn_enum(resolved_type: &ResolvedType) -> String {
  ...
  if let ResolvedTypeKind::Enum { variants, .. } = &resolved_type.kind {
    ...
    for variant in variants {
      ...
      write!(output, "void {}_set_{}( {}_t * self, {} const * value ) {{\n",
             type_name, variant_escaped, type_name, variant_type_name).unwrap();

      /* Variant data comes immediately after tag field */
      write!(output, "  memcpy( (unsigned char *)self + sizeof(uint8_t), value, sizeof( {} ) );\n",
             variant_type_name).unwrap();
      write!(output, "}}\n\n").unwrap();
    }
  }
