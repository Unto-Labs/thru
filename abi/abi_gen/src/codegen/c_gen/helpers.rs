...
pub fn format_expr_to_c(expr: &ExprKind, params: &[String]) -> String {
  match expr {
    ...
    // Binary operations
    ExprKind::Add(e) => format!("({}+{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Sub(e) => format!("({}-{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Mul(e) => format!("({}*{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Div(e) => format!("({}/{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Mod(e) => format!("({}%{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Pow(e) => {
      format!("pow({},{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params))
    }

    // Bitwise operations
    ExprKind::BitAnd(e) => {
      format!("({}&{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params))
    }
    ExprKind::BitOr(e) => format!("({}|{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::BitXor(e) => {
      format!("({}^{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params))
    }
    ...
    ExprKind::Popcount(e) => {
      format!("__builtin_popcount({})", format_expr_to_c(&e.operand, params))
    }

    // Comparison operations
    ExprKind::Eq(e) => format!("({}=={})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Ne(e) => format!("({}!={})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Lt(e) => format!("({}<{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Gt(e) => format!("({}>{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Le(e) => format!("({}<={})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Ge(e) => format!("({}>={})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),

    // Logical operations
    ExprKind::And(e) => format!("({}&&{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Or(e) => format!("({}||{})", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params)),
    ExprKind::Xor(e) => {
      format!("(({})!=({}))", format_expr_to_c(&e.left, params), format_expr_to_c(&e.right, params))
    }
  }
...
pub fn escape_c_keyword(name: &str) -> String {
  const C_KEYWORDS: &[&str] = &[
    ...
    "size_t", "ssize_t", "ptrdiff_t", "wchar_t", "NULL",
  ];
...
pub fn generate_nested_field_access(field_ref: &str, type_name: &str, field_prim_type: &PrimitiveType) -> String {
  ...
  write!(&mut output, "  int64_t {} = (int64_t)({}_get_{}( self ));\n", var_name, type_name, var_name).unwrap();
...
pub fn get_c_accessor_type(resolved_type: &ResolvedType) -> String {
  match &resolved_type.kind {
    ...
    _ => {
      ...
      panic!("get_c_accessor_type called with unsupported type: {:?}", resolved_type.kind)
    }
  }
}
