...
use crate::abi::expr::{ExprKind, ConstantExpression};
use crate::abi::types::{PrimitiveType, IntegralType};
use std::collections::{HashMap, BTreeMap, HashSet};
use std::fmt::Write;
use super::helpers::{escape_c_keyword, get_c_accessor_type, format_type_to_c, format_expr_to_c, generate_nested_field_access, is_nested_complex_type, primitive_to_c_type};
  ...
  for field in fields.iter() {
    ...
    match &field.field_type.kind {
      ResolvedTypeKind::Primitive { .. } => {
        ...
        write!(output, "{} {}_get_{}( {}_t const * self ) {{\n", field_type_str, type_name, escaped_name, type_name).unwrap();
        ...
        if after_variable_size_data {
          ...
          write!(output, "  {} value;\n", field_type_str).unwrap();
          write!(output, "  memcpy( &value, &self->{}, sizeof( value ) );\n", escaped_name).unwrap();
        }
      ...
      ResolvedTypeKind::Array { element_type, size_expression, .. } => {
        ...
        /* Const getter */
        write!(output, "{} const * {}_get_{}_const( {}_t const * self ) {{\n", element_c_type, type_name, escaped_name, type_name).unwrap();
        if after_variable_size_data {
          output.push_str(&fam_offset_code);
          write!(output, "  return ({} const *)((unsigned char const *)self + offset);\n", element_c_type).unwrap();
        } else {
        ...
        /* Mutable getter */
        write!(output, "{} * {}_get_{}( {}_t * self ) {{\n", element_c_type, type_name, escaped_name, type_name).unwrap();
        if after_variable_size_data {
          output.push_str(&fam_offset_code);
          write!(output, "  return ({} *)((unsigned char *)self + offset);\n", element_c_type).unwrap();
        } else {
          write!(output, "  return self->{};\n", escaped_name).unwrap();
        }
        write!(output, "}}\n\n").unwrap();
        ...
        if size_expression.is_constant() {
          ...
          write!(output, "  (void)self; /* unused for constant-size arrays */\n").unwrap();
          write!(output, "  return {};\n", size_expr).unwrap();
        } else {
          ...
          if let Size::Variable(field_map) = &field.field_type.size {
            if let Some(field_refs) = field_map.get(&field.name) {
              ...
              write!(output, "  return 0; /* ERROR: Could not determine array size */\n").unwrap();
            }
          } else {
            write!(output, "  return 0; /* ERROR: Variable size but no field references */\n").unwrap();
          }
        }
        write!(output, "}}\n\n").unwrap();
      }
      ResolvedTypeKind::Struct { .. } | ResolvedTypeKind::Union { .. } => {
        ...
        /* Const getter */
        write!(output, "{}_{}_inner_t const * {}_get_{}_const( {}_t const * self ) {{\n", type_name, escaped_name, type_name, escaped_name, type_name)
          .unwrap();

        if after_variable_size_data {
          output.push_str(&fam_offset_code);
          write!(output, "  return ({}_{}_inner_t const *)((unsigned char const *)self + offset);\n", type_name, escaped_name).unwrap();
        } else {
          write!(output, "  return &self->{};\n", escaped_name).unwrap();
        }
        write!(output, "}}\n\n").unwrap();
        ...
        /* Mutable getter */
        write!(output, "{}_{}_inner_t * {}_get_{}( {}_t * self ) {{\n", type_name, escaped_name, type_name, escaped_name, type_name)
          .unwrap();

        if after_variable_size_data {
          output.push_str(&fam_offset_code);
          write!(output, "  return ({}_{}_inner_t *)((unsigned char *)self + offset);\n", type_name, escaped_name).unwrap();
        } else {
          write!(output, "  return &self->{};\n", escaped_name).unwrap();
        }
        write!(output, "}}\n\n").unwrap();
        ...
        if let ResolvedTypeKind::Struct { fields: nested_fields, .. } = &field.field_type.kind {
          for nested_field in nested_fields {
            if let ResolvedTypeKind::Primitive { prim_type } = &nested_field.field_type.kind {
              ...
              let parent_accessor_name = format!("{}_{}", escaped_name, nested_escaped);
              ...
              write!(output, "{} {}_get_{}( {}_t const * self ) {{\n",
                     field_type_str, type_name, parent_accessor_name, type_name).unwrap();
              write!(output, "  {} value;\n", field_type_str).unwrap();
              write!(output, "  memcpy( &value, &self->{}.{}, sizeof( value ) );\n",
                     escaped_name, nested_escaped).unwrap();
              write!(output, "  return value;\n").unwrap();
              write!(output, "}}\n\n").unwrap();
            }
          }
      ...
      ResolvedTypeKind::TypeRef { target_name, .. } => {
        ...
        /* Const getter */
        write!(output, "{}_t const * {}_get_{}_const( {}_t const * self ) {{\n", target_name, type_name, escaped_name, type_name).unwrap();

        if after_variable_size_data {
          output.push_str(&fam_offset_code);
          write!(output, "  return ({}_t const *)((unsigned char const *)self + offset);\n", target_name).unwrap();
        } else {
          write!(output, "  return &self->{};\n", escaped_name).unwrap();
        }
        write!(output, "}}\n\n").unwrap();
        ...
        /* Mutable getter */
        write!(output, "{}_t * {}_get_{}( {}_t * self ) {{\n", target_name, type_name, escaped_name, type_name).unwrap();

        if after_variable_size_data {
          output.push_str(&fam_offset_code);
          write!(output, "  return ({}_t *)((unsigned char *)self + offset);\n", target_name).unwrap();
        } else {
          write!(output, "  return &self->{};\n", escaped_name).unwrap();
        }
        write!(output, "}}\n\n").unwrap();
      ...
      ResolvedTypeKind::Enum { variants, .. } => {
        ...
        for variant in variants {
          ...
          write!(output, "  return ({} const *){}_get_{}( self );\n",
                 variant_type_name, type_name, escaped_name).unwrap();
          write!(output, "}}\n\n").unwrap();

          /* Generate mutable getter - name includes enum field name */
          write!(output, "{} * {}_{}_get_{}( {}_t * self ) {{\n",
                 variant_type_name, type_name, escaped_name, variant_escaped, type_name).unwrap();
          write!(output, "  return ({} *)(void *){}_get_{}( ({}_t const *)self );\n",
                 variant_type_name, type_name, escaped_name, type_name).unwrap();
          write!(output, "}}\n\n").unwrap();
        }
      }
            ...
            if let Size::Variable(variable_refs) = &resolved_type.size {
              if let Some(field_map) = variable_refs.get(&field.name) {
                ...
                for (field_ref, prim_type) in field_map {
                  ...
                  if declared_refs.insert(field_ref.clone()) {
                    fam_offset_code.push_str(&generate_nested_field_access(field_ref_str, type_name, prim_type));
                  }
                }
                ...
                if !variant_ref_map.is_empty() {
                  for field_ref in &variant_ref_order {
                    if declared_refs.insert(field_ref.clone()) {
                      ...
                      write!(fam_offset_code, "  int64_t {} = 0;\n", var_name).unwrap();
                    }
                  }
                ...
                let mut params: Vec<String> = field_map.keys()
                  .map(|field_ref| field_ref.replace(".", "_"))
                  .collect();
                params.sort();
                write!(fam_offset_code, "  offset += {}_{}_inner_footprint( {} );\n", type_name, field.name, params.join(", ")).unwrap();
              }
            }
          } else {
            /* Const-size enum */
          ...
          if let Size::Variable( .. ) = &field.field_type.size {
            if let Size::Variable(field_map) = &field.field_type.size {
              if let Some(field_refs) = field_map.get(&field.name) {
                ...
                write!(fam_offset_code, "  offset += {}_{}_footprint( {} );\n", type_name, field.name, params.join(", ")).unwrap();
              }
            }
          } else {
            write!(fam_offset_code, "  offset += sizeof( {}_t );\n", target_name).unwrap();
          }
        }
      }
    }
...
pub fn emit_accessor_fn_union(resolved_type: &ResolvedType) -> String {
  ...
  if let ResolvedTypeKind::Union { variants } = &resolved_type.kind {
    for variant in variants {
      ...
      match &variant.field_type.kind {
        ResolvedTypeKind::Primitive { .. } => {
          ...
          writeln!(output, "{} {}_get_{}( {}_t const * self ) {{", c_type, type_name, escaped_variant_name, type_name).unwrap();
          writeln!(output, "  {} value;", c_type).unwrap();
          writeln!(output, "  memcpy( &value, &self->{}, sizeof( value ) );", escaped_variant_name).unwrap();
        ...
        ResolvedTypeKind::Array { element_type, .. } => {
          ...
          writeln!(output, "{} {}_get_{}( {}_t const * self ) {{", return_type, type_name, escaped_variant_name, type_name).unwrap();
        ...
        ResolvedTypeKind::TypeRef { target_name, .. } => {
          ...
          writeln!(output, "{} {}_get_{}_const( {}_t const * self ) {{", const_return_type, type_name, escaped_variant_name, type_name).unwrap();
          writeln!(output, "  return &self->{};", escaped_variant_name).unwrap();
          writeln!(output, "}}\n").unwrap();

          /* Mutable getter */
          let mut_return_type = format!("{}_t *", target_name);
          writeln!(output, "{} {}_get_{}( {}_t * self ) {{", mut_return_type, type_name, escaped_variant_name, type_name).unwrap();
          writeln!(output, "  return &self->{};", escaped_variant_name).unwrap();
          writeln!(output, "}}\n").unwrap();
        }
        _ => {
          /* Const getter */
          let const_return_type = format!("{}_{}_inner_t const *", type_name, escaped_variant_name);
          writeln!(output, "{} {}_get_{}_const( {}_t const * self ) {{", const_return_type, type_name, escaped_variant_name, type_name).unwrap();
          writeln!(output, "  return &self->{};", escaped_variant_name).unwrap();
          writeln!(output, "}}\n").unwrap();

          /* Mutable getter */
          let mut_return_type = format!("{}_{}_inner_t *", type_name, escaped_variant_name);
          writeln!(output, "{} {}_get_{}( {}_t * self ) {{", mut_return_type, type_name, escaped_variant_name, type_name).unwrap();
          writeln!(output, "  return &self->{};", escaped_variant_name).unwrap();
          writeln!(output, "}}\n").unwrap();
        }
    ...
    writeln!(output, "void const * {}_get_variant( {}_t const * self ) {{", type_name, type_name).unwrap();
    writeln!(output, "  /* WARNING: unchecked accessor; caller must know which variant is active */").unwrap();
