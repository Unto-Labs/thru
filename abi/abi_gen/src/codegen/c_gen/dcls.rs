...
use super::helpers::{
  escape_c_keyword, format_type_to_c, get_c_accessor_type, is_nested_complex_type, primitive_to_c_type,
};
...
pub fn emit_forward_declarations(resolved_type: &ResolvedType) -> String {
  ...
  match &resolved_type.kind {
    ResolvedTypeKind::Struct { fields, .. } => {
      ...
      output.push_str(&format!("{}_t const * {}_from_slice( uint8_t const * data, uint64_t data_len );\n",
                               type_name, type_name));

      /* from_slice_mut() - returns mutable pointer to opaque type */
      output.push_str(&format!("{}_t * {}_from_slice_mut( uint8_t * data, uint64_t data_len );\n",
                               type_name, type_name));
      ...
      output.push_str(&format!("int {}_new( uint8_t * buffer, uint64_t buffer_size", type_name));
      ...
      for field in fields.iter() {
        if let ResolvedTypeKind::Primitive { prim_type } = &field.field_type.kind {
          let c_type = primitive_to_c_type(prim_type);
          output.push_str(&format!("{} {}_get_{}( {}_t const * self );\n",
      ...
      for field in fields.iter() {
        if let ResolvedTypeKind::Enum { .. } = &field.field_type.kind {
          output.push_str(&format!("uint64_t {}_get_{}_size( {}_t const * self );\n",
      ...
      for field in fields.iter() {
        if let ResolvedTypeKind::Enum { .. } = &field.field_type.kind {
          output.push_str(&format!("uint8_t const * {}_get_{}_body( {}_t const * self );\n",
                                 type_name, field.name, type_name));
      ...
      for field in fields.iter() {
        if let ResolvedTypeKind::Primitive { prim_type } = &field.field_type.kind {
          ...
          let c_type = primitive_to_c_type(prim_type);
          output.push_str(&format!("void {}_set_{}( {}_t * self, {} value );\n",
          ...
          if let crate::abi::resolved::Size::Const(array_size) = field.field_type.size {
            if let ResolvedTypeKind::Primitive { prim_type } = &element_type.kind {
              ...
              if array_size % elem_size == 0 {
                /* Length getter */
                output.push_str(&format!("uint64_t {}_get_{}_length( {}_t const * self );\n",
                                       type_name, field.name, type_name));

                /* Index getter */
                output.push_str(&format!("{} {}_get_{}_at( {}_t const * self, uint64_t index );\n",
                                       elem_c_type, type_name, field.name, type_name));

                /* Index setter */
                output.push_str(&format!("void {}_set_{}_at( {}_t * self, uint64_t index, {} value );\n",
      ...
      for field in fields.iter() {
        if let ResolvedTypeKind::TypeRef { target_name, ..} = &field.field_type.kind {
          if let Size::Const(_nested_size) = field.field_type.size {
            ...
            output.push_str(&format!("{}_t const * {}_get_{}_const( {}_t const * self );\n",
                                   target_name, type_name, field.name, type_name));

            /* Mutable getter - returns mutable pointer */
            output.push_str(&format!("{}_t * {}_get_{}( uint8_t * data );\n",
                                   target_name, type_name, field.name));

            /* Setter - accepts wrapper by const pointer */
            output.push_str(&format!("int {}_set_{}( {}_t * self, {}_t const * nested );\n",
              ...
              ResolvedTypeKind::Array { element_type, .. } => {
                if let ResolvedTypeKind::Primitive { prim_type } = &element_type.kind {
                  if !matches!(nested_field.field_type.size, crate::abi::resolved::Size::Const(..)) {
                    ...
                    output.push_str(&format!("uint64_t {}_get_{}_{}_length( {}_t const * self );\n",
    ...
    ResolvedTypeKind::Union { .. } | ResolvedTypeKind::Enum { .. } => {
      ...
      match &resolved_type.kind {
        ResolvedTypeKind::Struct { fields, .. } => {
            ...
            match &field.field_type.kind {
              ...
              ResolvedTypeKind::Array { element_type, .. } => {
                ...
                if is_nested_complex_type(element_type) {
                  element_param_type = format!("{}_{}_inner_t", type_name, field_name);
                }
              ...
              _ => {
                ...
                if matches!(&field.field_type.kind, ResolvedTypeKind::Enum { .. }) {
                  ...
                  if is_nested_complex_type(&field.field_type) {
                    pointer_type = format!("{}_{}_inner_t", type_name, field_name);
                  }
                  if is_fam {
                    let size_param_name = format!("{}_sz", param_name);
                    field_param_lines.push(format!(
                    ...
                    field_param_lines.push(format!("{} const * {}", pointer_type, param_name));
                  }
                }
              }
            }
          ...
          if field_param_lines.is_empty() {
            output.push_str(&format!("int {}_init( void * buffer, uint64_t buf_sz );\n", type_name));
          } else {
            output.push_str(&format!("int {}_init( void * buffer, uint64_t buf_sz,\n", type_name));
          ...
          output.push_str(&format!("uint64_t {}_size( {}_t const * self );\n", type_name, type_name));
        }
      ...
      if !matches!(resolved_type.kind, ResolvedTypeKind::Struct { .. } | ResolvedTypeKind::Union { .. }) {
        output.push_str(&format!("uint64_t {}_size( {}_t const * self );\n", type_name, type_name));
      }
        ...
        for field in fields {
          ...
          match &field.field_type.kind {
            ...
            ResolvedTypeKind::TypeRef { target_name, .. } => {
              ...
              output.push_str(&format!("void {}_set_{}( {}_t * self, {}_t const * value );\n", type_name, escaped_name, type_name, target_name));
            }
            ResolvedTypeKind::Enum { variants, .. } => {
              /* For enum fields, emit generic body getter/setter */
              output.push_str(&format!("void const * {}_get_{}( {}_t const * self );\n", type_name, escaped_name, type_name));
              output.push_str(&format!("void {}_set_{}( {}_t * self, void const * value );\n", type_name, escaped_name, type_name));
        ...
        for field in fields {
          ...
          match &field.field_type.kind {
            ...
            ResolvedTypeKind::Array { element_type, .. } => {
              ...
              /* Const getter */
              output.push_str(&format!("{} const * {}_get_{}_const( {}_t const * self );\n",
                element_c_type, type_name, escaped_name, type_name));

              /* Mutable getter */
              output.push_str(&format!("{} * {}_get_{}( {}_t * self );\n",
                element_c_type, type_name, escaped_name, type_name));
            ...
            ResolvedTypeKind::TypeRef { target_name, .. } => {
              ...
              /* Mutable getter */
              output.push_str(&format!("{}_t * {}_get_{}( {}_t * self );\n", target_name, type_name, escaped_name, type_name));
            }
            ResolvedTypeKind::Enum { variants, .. } => {
