...
use super::helpers::{escape_c_keyword, primitive_to_c_type, format_type_to_c, format_expr_to_c, is_nested_complex_type};
use std::fmt::Write;
...
fn emit_init_fn_struct(resolved_type: &ResolvedType) -> String {
  ...
  #[derive(Clone)]
  enum FieldInitKind {
    Primitive { size_expr: String },
    Array { len_name: String, elem_size_expr: String },
    ConstPointer { size_expr: String },
    VarPointer { size_param_name: String },
  }
    ...
    match &field.field_type.kind {
      ResolvedTypeKind::Primitive { prim_type } => {
        ...
        field_infos.push(FieldInitInfo {
          ...
          init_kind: FieldInitKind::Primitive { size_expr: format!("sizeof( {} )", type_str) },
          is_fam,
        });
      }
      ResolvedTypeKind::Array { element_type, .. } => {
        ...
        field_param_lines.push(format!("{} const * {}, uint64_t {}", element_param_type, param_name.clone(), len_name.clone()));
      ...
      _ => {
        ...
        if matches!(&field.field_type.kind, ResolvedTypeKind::Enum { .. }) {
          ...
          field_param_lines.push(format!("void const * {}, uint64_t {}", param_name.clone(), size_param_name.clone()));
          field_infos.push(FieldInitInfo {
            raw_name: field.name.clone(),
            param_name: param_name.clone(),
          ...
          if is_fam {
            ...
            let size_param_name = format!("{}_sz", param_name);
            field_param_lines.push(format!("{} const * {}, uint64_t {}", pointer_type, param_name.clone(), size_param_name.clone()));
            field_infos.push(FieldInitInfo {
              raw_name: field.name.clone(),
              param_name: param_name.clone(),
              init_kind: FieldInitKind::VarPointer { size_param_name },
            ...
            field_param_lines.push(format!("{} const * {}", pointer_type, param_name.clone()));
            field_infos.push(FieldInitInfo {
              raw_name: field.name.clone(),
              param_name: param_name.clone(),
              init_kind: FieldInitKind::ConstPointer { size_expr: format!("sizeof( {} )", pointer_type) },
              is_fam,
            });
          }
        }
  ...
  if field_param_lines.is_empty() {
    write!(output, "int {}_init( void * buffer, uint64_t buf_sz ) {{\n", type_name).unwrap();
  } else {
    write!(output, "int {}_init( void * buffer, uint64_t buf_sz,\n", type_name).unwrap();
  ...
  for info in &field_infos {
    if info.is_fam && !after_variable_size_data {
      ...
      write!(output, "  uint64_t offset = offsetof( {}_t, {} );\n", type_name, info.raw_name).unwrap();
    }
    if !after_variable_size_data {continue;}
    match &info.init_kind {
        FieldInitKind::Primitive { size_expr } => {
            ...
            write!(output, "    if( safe_add_u64( offset, field_bytes, &offset ) ) return -1;\n").unwrap();
            write!(output, "  }}\n").unwrap();
      }
      FieldInitKind::Array { len_name, elem_size_expr } => {
        let elem_size = format!("(uint64_t)({})", elem_size_expr);
            write!(output, "  {{  /* field: {} */\n", info.raw_name).unwrap();
            write!(output, "    uint64_t elem_size = {};\n", elem_size).unwrap();
            write!(output, "    uint64_t field_bytes = 0;\n").unwrap();
            write!(output, "    if( safe_mul_u64( elem_size, {}, &field_bytes ) ) return -1;\n", len_name).unwrap();
            write!(output, "    if( safe_add_u64( offset, field_bytes, &offset ) ) return -1;\n").unwrap();
            write!(output, "  }}\n").unwrap();
      ...
      FieldInitKind::VarPointer { size_param_name } => {
            write!(output, "  {{  /* field: {} (variable-sized) */\n", info.raw_name).unwrap();
  ...
  for field in fields {
    if matches!(&field.field_type.size, Size::Variable(_)) {
      if !has_variable_size_data {
        ...
        first_variable_size_is_enum = matches!(&field.field_type.kind, ResolvedTypeKind::Enum { .. });
      }
      break;
    }
  }
  ...
  write!(output, "  {}_t * self = ({}_t *)buffer;\n", type_name, type_name).unwrap();
    ...
    match &info.init_kind {
      FieldInitKind::Primitive { size_expr } => {
        ...
        if after_variable_size_data {
            write!(output, "  {{  /* field: {} */\n", info.raw_name).unwrap();
            write!(output, "    uint64_t field_bytes = {};\n", field_size).unwrap();
            write!(output, "    memcpy( (unsigned char *)self + offset, &{}, field_bytes );\n", info.param_name).unwrap();
            write!(output, "    offset += field_bytes;\n").unwrap();
            write!(output, "  }}\n").unwrap();
        }else{
          write!(output, "  memcpy( &self->{}, &{}, {} ); // field: {}\n", info.raw_name, info.param_name, field_size, info.raw_name).unwrap();
        }
      }
      FieldInitKind::Array { len_name, elem_size_expr } => {
        let elem_size = format!("(uint64_t)({})", elem_size_expr);
        if after_variable_size_data {
          ...
          write!(output, "    uint64_t field_bytes = 0;\n").unwrap();
          write!(output, "    if( safe_mul_u64( elem_size, {}, &field_bytes ) ) return -1;\n", len_name).unwrap();
          write!(output, "    memcpy( self->{}, {}, field_bytes );\n", info.raw_name, info.param_name).unwrap();
          write!(output, "  }}\n").unwrap();
        }
      }
      FieldInitKind::ConstPointer { size_expr } => {
      ...
      FieldInitKind::VarPointer { size_param_name } => {
        if after_variable_size_data {
          ...
          write!(output, "  memcpy( &self->{}, {}, {} ); // field: {} (variable-sized)\n", info.raw_name, info.param_name, size_param_name, info.raw_name).unwrap();
        }
      }
    }
  }

  write!(output, "  int err = {}_validate( buffer, buf_sz, NULL );\n", type_name).unwrap();
...
fn emit_init_fn_union(resolved_type: &ResolvedType) -> String {
  ...
  for variant in variants {
    ...
    write!(output, "int {}_init_{}( void * buffer, uint64_t buf_sz, {} ) {{\n", type_name, escaped_variant, param_decl)
      .unwrap();
    write!(output, "  if( sizeof( {}_t ) > buf_sz ) {{\n", type_name).unwrap();
    write!(output, "    return -1; /* Buffer too small */\n").unwrap();
    write!(output, "  }}\n").unwrap();
    ...
    match &variant.field_type.kind {
      ResolvedTypeKind::Primitive { .. } => {
        write!(output, "  memcpy( &self->{}, &value, sizeof( self->{} ) );\n", escaped_variant, escaped_variant)
          .unwrap();
      }
      ResolvedTypeKind::Array { .. } => {
        ...
        write!(output, "  memcpy( self->{}, value, len * sizeof self->{}[0] );\n", escaped_variant, escaped_variant)
          .unwrap();
      }
      _ => {
        write!(output, "  memcpy( &self->{}, value, sizeof( self->{} ) );\n", escaped_variant, escaped_variant)
