...
impl ReflectedValue {
  ...
  pub fn get_struct_field(&self, field_name: &str) -> Option<&ReflectedValue> {
    if let Value::Struct { fields } = &self.value {
      fields.iter().find(|(name, _)| name == field_name).map(|(_, value)| value)
    } else {
      None
    }
  }
...
impl ValueOnly {
  ...
  pub fn from_reflected_value(reflected: &ReflectedValue) -> Self {
    match &reflected.value {
      ...
      Value::Union { variant_name, variant_value } => {
        ValueOnly::Union {
          variant: variant_name.clone(),
          value: Box::new(ValueOnly::from_reflected_value(variant_value)),
        }
      }
      Value::Enum { variant_name, variant_value, .. } => {
        ValueOnly::Enum {
          variant: variant_name.clone(),
          value: Box::new(ValueOnly::from_reflected_value(variant_value)),
        }
      ...
      Value::Array { elements } => {
        ValueOnly::Array(elements.iter().map(ValueOnly::from_reflected_value).collect())
      }
      Value::SizeDiscriminatedUnion { variant_name, variant_value } => {
