    ...
    let kind = match &resolved.kind {
      ...
      ResolvedTypeKind::Struct { fields, packed, custom_alignment } => {
        ReflectedTypeKind::Struct {
          fields: fields.iter().map(|f| ReflectedField::from_resolved(f)).collect(),
          packed: *packed,
          custom_alignment: *custom_alignment,
        }
      }
      ResolvedTypeKind::Union { variants } => ReflectedTypeKind::Union {
        variants: variants.iter().map(|f| ReflectedField::from_resolved(f)).collect(),
      },
      ResolvedTypeKind::Enum { tag_expression, tag_constant_status, variants } => {
        ReflectedTypeKind::Enum {
          ...
          variants: variants.iter().map(|v| ReflectedEnumVariant::from_resolved(v)).collect(),
        }
      }
      ...
      ResolvedTypeKind::SizeDiscriminatedUnion { variants } => {
        ReflectedTypeKind::SizeDiscriminatedUnion {
          variants: variants.iter().map(|v| ReflectedSizeDiscriminatedVariant::from_resolved(v)).collect(),
        }
      }
      ResolvedTypeKind::TypeRef { target_name, resolved } => {
...
impl ReflectedSizeDiscriminatedVariant {
  ...
  pub fn from_resolved(variant: &abi_gen::abi::resolved::ResolvedSizeDiscriminatedVariant) -> Self {
    Self {
      name: variant.name.clone(),
