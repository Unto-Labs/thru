use super::helpers::{format_expr_to_c, generate_nested_field_access, sanitize_type_name};
...
fn emit_size_fn_struct(resolved_type: &ResolvedType) -> String {
    ...
    match &resolved_type.size {
        ...
        Size::Variable(variable_refs) => {
            ...
            let param_names: Vec<String> = non_constant_refs
                .iter()
                    ...
                    if let Some(field_map) = variable_refs.get(&field.name) {
                        match &field.field_type.kind {
                            ResolvedTypeKind::Enum {
                                ...
                                let mut variant_ref_map: HashMap<String, Vec<String>> =
                                    HashMap::new();
                                let mut variant_ref_order: Vec<String> = Vec::new();

                                for field_ref in field_map.keys() {
                            ...
                            _ => {
                                let mut ordered_refs: Vec<String> =
                                    field_map.keys().cloned().collect();
                                ordered_refs.sort();
                                for field_ref in ordered_refs {
                                    if declared_refs.insert(field_ref.clone()) {
                                        output.push_str(&generate_nested_field_access(
                                            field_ref.as_str(),
                                            type_name_str,
