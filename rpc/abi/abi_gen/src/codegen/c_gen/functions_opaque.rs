...
use super::ir_footprint::sanitize_symbol;
...
fn emit_read_primitive_direct(
    ...
    match prim_type {
        ...
        _ => {
            ...
            write!(
                output,
                "({{ {} val; memcpy( &val, &{}[{}], sizeof( val ) ); val; }})",
                c_type, data_ptr, offset_expr
            )
            .unwrap();
        }
    }
    ...
    match prim_type {
        PrimitiveType::Integral(IntegralType::U8) | PrimitiveType::Integral(IntegralType::I8) => {
            write!(
                output,
                "    {}[{}] = {};\n",
                data_ptr, offset_expr, value_expr
            )
            .unwrap();
        }
        _ => {
            ...
            write!(
                output,
                "    memcpy( &{}[{}], &{}, sizeof( {} ) );\n",
                data_ptr, offset_expr, value_expr, value_expr
            )
            .unwrap();
        }
    }
...
fn identify_size_affecting_fields(
...
pub fn emit_opaque_functions(resolved_type: &ResolvedType) -> String {
    ...
    match &resolved_type.kind {
        ResolvedTypeKind::Struct { fields, .. } => {
            ...
            write!(
                output,
                "    if( {}_validate( data, data_len, &required_size ) != 0 ) {{\n",
                type_name
            )
            .unwrap();
            ...
            if !is_nested {
                ...
                for field in fields {
                    if matches!(
                        ...
                        write!(output, ", uint64_t {}_payload_size", field.name).unwrap();
                    }
                }
                ...
                for field in fields.iter() {
                    match &field.field_type.kind {
                            ...
                            for variant in variants {
                                    ...
                                    variant.variant_type.size
                                {
                                    write!(
                                        output,
                                        "        case {}: {}_size = {}; break;\n",
                                        variant.tag_value, field.name, size
                                    )
                                    .unwrap();
                                }
                            }
                        ...
                        ResolvedTypeKind::SizeDiscriminatedUnion { variants } => {
                            /* Size-discriminated union: size is determined from payload size parameter */
                            let payload_param = format!("{}_payload_size", field.name);
                            write!(
                                output,
                                "    /* Size-discriminated union '{}' size based on tag */\n",
                                field.name
                            )
                            .unwrap();
                            write!(
                                output,
                                "    uint8_t {}_tag = {}_{}_tag_from_size( (uint64_t){} );\n",
                                field.name, type_name, field.name, payload_param
                            )
                            .unwrap();
                            write!(
                                output,
                                "    if( {}_tag == 0xFF ) return -1; /* Invalid payload size */\n",
                                field.name
                            )
                            .unwrap();
                            write!(output, "    uint64_t {}_size;\n", field.name).unwrap();
                            write!(output, "    switch( {}_tag ) {{\n", field.name).unwrap();
                            for (idx, variant) in variants.iter().enumerate() {
                                write!(
                                    output,
                                    "        case {}: {}_size = {}; break;\n",
                                    idx, field.name, variant.expected_size
                                )
                                .unwrap();
                            }
                        ...
                        ResolvedTypeKind::Struct {
                            ...
                            if let crate::abi::resolved::Size::Const(size) = field.field_type.size {
                                ...
                                write!(
                                    output,
                                    "    /* Calculate variable-size inline nested struct '{}' */\n",
                                    field.name
                                )
                                .unwrap();
                                for nested_field in nested_fields {
                                        ...
                                        ResolvedTypeKind::Array {
                                                ...
                                                nested_field.field_type.size
                                            {
                                                ...
                                                ) = element_type.size
                                                {
                                                    ...
                                                    let size_calc =
                                                        format_expr_to_c(size_expression, &params);
                                                    write!(output, "    required_size += ({}) * {}; /* {}.{} (variable array) */\n",
                ...
                for field in fields.iter() {
                    match &field.field_type.kind {
                        ...
                        ResolvedTypeKind::Enum { .. } => {
                            ...
                            write!(
                                output,
                                "    offset += {}_size; /* skip enum '{}' (set via setters) */\n\n",
                                field.name, field.name
                            )
                            .unwrap();
                        }
                        ...
                        ResolvedTypeKind::Array {
                            ...
                            if let crate::abi::resolved::Size::Const(size) = field.field_type.size {
                                write!(
                                    output,
                                    "    offset += {}; /* skip array '{}' (set via setters) */\n\n",
                                    size, field.name
                                )
                                .unwrap();
                            } else {
                ...
                match &field.field_type.kind {
                                    ...
                                    ResolvedTypeKind::Array {
                                            ...
                                            prev_field.field_type.size
                                        {
                                            ...
                                            if let crate::abi::resolved::Size::Const(elem_size) =
                                                element_type.size
                                            {
                                                let size_calc = size_expression_to_c_getter_code(
                                                    size_expression,
                                                    &type_name,
                                                    "self",
                                                );
                    ...
                    ResolvedTypeKind::Array { element_type, .. } => {
                        ...
                        if let crate::abi::resolved::Size::Const(array_size) = field.field_type.size
                        {
                            ...
                            let (elem_c_type, elem_size, is_primitive) = match &element_type.kind {
                                ResolvedTypeKind::Primitive { prim_type } => (
                                    primitive_to_c_type(prim_type).to_string(),
                            ...
                            if is_primitive {
                                    ...
                                    &element_type.kind
                                {
                                    write!(
                                        output,
                                        "    {}_t const * result = ({}_t const *)&data[offset];\n",
                            ...
                            } = &field.field_type.kind
                            {
                                ...
                                let size_expr = size_expression_to_c_getter_code(
                                    size_expression,
                                    &type_name,
                                    "self",
                                );
                                ...
                                write!(
                                    output,
                                    "/* Variable-size array accessor helpers for {} */\n",
                                    field.name
                                )
                                .unwrap();
                                ...
                                /* Extract element type info */
                                let (elem_c_type, elem_size, is_primitive) = match &element_type
                                    .kind
                                {
                                    ResolvedTypeKind::Primitive { prim_type } => (
                                        primitive_to_c_type(prim_type).to_string(),
                    ...
                    ResolvedTypeKind::TypeRef { target_name, .. } => {
                            ...
                            field.field_type.size
                        {
                            ...
                            write!(
                                output,
                                "/* Nested struct const getter for {} */\n",
                                field.name
                            )
                            .unwrap();
                            write!(
                            ...
                            write!(
                                output,
                                "/* Nested struct mutable getter for {} */\n",
                                field.name
                            )
                            .unwrap();
                            write!(
                            ...
                            write!(
                                output,
                                "    memcpy( &data[offset], (uint8_t const *)nested, {} );\n",
                                nested_size
                            )
                            .unwrap();
                            write!(output, "    return 0; /* Success */\n").unwrap();
            ...
            for field in fields {
                ...
                } = &field.field_type.kind
                {
                    ...
                    let tag_expr =
                        size_expression_to_c_getter_code(tag_expression, &type_name, "self");
            ...
            for (field_idx, field) in fields.iter().enumerate() {
                if let ResolvedTypeKind::Enum { .. } = &field.field_type.kind {
                    ...
                    write!(
                        output,
                        "/* Generic body getter for enum field '{}' */\n",
                        field.name
                    )
                    .unwrap();
                    write!(
                        ...
                        for prev_field in &fields[0..field_idx] {
                            match &prev_field.field_type.kind {
                                ...
                                ResolvedTypeKind::Enum { .. } => {
                                    write!(
                                        output,
                                        "    offset += {}_get_{}_size( self ); /* {} (enum) */\n",
                                        type_name, prev_field.name, prev_field.name
                                    )
                                    .unwrap();
                                }
                                ResolvedTypeKind::Array { .. } => {
                    ...
                    write!(
                        output,
                        "/* Generic body setter for enum field '{}' */\n",
                        field.name
                    )
                    .unwrap();
                    ...
                    write!(
                        output,
                        "    uint64_t expected_size = {}_get_{}_size( ({}_t const *)self );\n",
                        type_name, field.name, type_name
                    )
                    .unwrap();
            ...
            for (field_idx, field) in fields.iter().enumerate() {
                    ...
                    &field.field_type.kind
                {
                    ...
                    for variant in variants {
                        ...
                        write!(
                            output,
                            "    memcpy( &data[offset], value, sizeof( {} ) );\n",
                            variant_type_name
                        )
                        .unwrap();
                        write!(output, "}}\n\n").unwrap();
            ...
            for (field_idx, field) in fields.iter().enumerate() {
                match &field.field_type.kind {
                    ...
                    ResolvedTypeKind::Array { element_type, .. } => {
                        ...
                        if let crate::abi::resolved::Size::Const(_) = field.field_type.size {
                            /* Extract element type info */
                            let (elem_c_type, elem_size, is_primitive) = match &element_type.kind {
                                ResolvedTypeKind::Primitive { prim_type } => (
                                    primitive_to_c_type(prim_type).to_string(),
                            ...
                            if is_primitive {
                                ...
                                write!(
                                    output,
                                    "    memcpy( &data[offset], (uint8_t const *)value, {} );\n",
                                    elem_size
                                )
                                .unwrap();
                            }
                            ...
                            if let ResolvedTypeKind::Array { element_type, .. } =
                                &field.field_type.kind
                            {
                                /* Extract element type info */
                                let (elem_c_type, elem_size, is_primitive) = match &element_type
                                ...
                                write!(
                                    output,
                                    "    uint64_t offset = base_offset + index * {};\n",
                                    elem_size
                                )
                                .unwrap();
            ...
            for (field_idx, field) in fields.iter().enumerate() {
                match &field.field_type.kind {
                    ...
                    ResolvedTypeKind::Enum {
                        ...
                        for variant in variants {
                                ...
                                variant.variant_type.size
                            {
                                write!(
                                    output,
                                    "        case {}: {} = {}; break;\n",
                                    variant.tag_value, size_var, size
                                )
                                .unwrap();
                            }
                        }
                        ...
                        write!(
                            output,
                            "        return -1; /* Buffer too small for enum '{}' */\n",
                            field.name
                        )
                        .unwrap();
                        write!(output, "    }}\n").unwrap();
                    ...
                    ResolvedTypeKind::SizeDiscriminatedUnion { variants } => {
                        ...
                        write!(
                            output,
                            "        default: return -1; /* No matching variant for size {} */\n",
                            available_size_var
                        )
                        .unwrap();
                        write!(output, "    }}\n\n").unwrap();
                    ...
                    ResolvedTypeKind::Array { .. } => {
                        if let crate::abi::resolved::Size::Const(size) = field.field_type.size {
                            ...
                            write!(
                                output,
                                "        return -1; /* Buffer too small for array '{}' */\n",
                                field.name
                            )
                            .unwrap();
                            write!(output, "    }}\n").unwrap();
                    ...
                    ResolvedTypeKind::TypeRef { .. } => {
                        if let crate::abi::resolved::Size::Const(size) = field.field_type.size {
                            ...
                            write!(
                                output,
                                "        return -1; /* Buffer too small for nested '{}' */\n",
                                field.name
                            )
                            .unwrap();
                            write!(output, "    }}\n").unwrap();
                    ...
                    ResolvedTypeKind::Struct {
                        ...
                        if let crate::abi::resolved::Size::Const(size) = field.field_type.size {
                            ...
                            write!(
                                output,
                                "    uint64_t {} = offset; /* Save offset for '{}' */\n",
                            ...
                            for nested_field in nested_fields {
                                    ...
                                    &nested_field.field_type.kind
                                {
                                    ...
                                    let absolute_offset = if nested_offset_within_struct == 0 {
                                        ...
                                        format!(
                                            "{} + {}",
                                            nested_struct_offset_var, nested_offset_within_struct
                                        )
                                    };
                                    ...
                                    ResolvedTypeKind::Array {
                                            ...
                                            nested_field.field_type.size
                                        {
                                            ...
                                            if let crate::abi::resolved::Size::Const(elem_size) =
                                                element_type.size
                                            {
                                                let size_expr = expression_to_c_data_read(
                                                    size_expression,
                                                    &field_offsets,
                                                );
                                                write!(
                                                    output,
                                                    "    uint64_t array_count_{} = ({});\n",
                        ...
                        match &nested_field.field_type.kind {
                            ResolvedTypeKind::Primitive { prim_type } => {
                                ...
                                if !referenced_fields.contains(&field_path) {
                                    ...
                                    write!(
                                        output,
                                        "void {}_set_{}_{}( uint8_t * data, {} value ) {{\n",
                                        type_name, field.name, nested_field.name, prim_c_type
                                    )
                                    .unwrap();
                                    /* Calculate offset to this primitive field */
                                    write!(output, "    uint64_t offset = 0;\n").unwrap();
