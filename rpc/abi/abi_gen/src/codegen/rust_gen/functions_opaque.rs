...
fn emit_write_primitive(
    ...
    match prim_type {
        PrimitiveType::Integral(int_type) => match int_type {
            IntegralType::U8 | IntegralType::Char => {
                format!("self.data[{}] = {};", offset_expr, value_expr)
            }
                            ...
                            } = &field.field_type.kind
                            {
                                ...
                                if *jagged {
                                        ...
                                        &element_type.kind
                                    {
                                        ...
                                        let count_expr = size_expression_to_rust_getter_code(
                                            size_expression,
                                            "self",
                                        );
                                    ...
                                    &element_type.kind
                                {
                                    ...
                                    {
                                        // Check if size expression would collide with array _len() method name
                                        let skip_len_method = size_expr_matches_len_field(
                                            size_expression,
                                            &field.name,
                                        );
                        ...
                        if matches!(size_constant_status, ConstantStatus::Constant) {
                            ...
                            } = &field.field_type.kind
                            {
                                    ...
                                    &element_type.kind
                                {
                                    ...
                                    // Convert expression to Rust code that calls getters
                                    let size_expr_str = size_expression_to_rust_getter_code(
                                        size_expression,
                                        "self",
                                    );
...
fn emit_jagged_array_accessors(
    ...
    writeln!(
        output,
        "    /// Returns an iterator over the jagged array elements."
    )
    .unwrap();
    writeln!(
        output,
        "    /// This is more efficient than repeated calls to `{}_get()` for sequential access.",
        field_name
    )
    .unwrap();
    writeln!(
...
fn emit_jagged_array_mut_accessors(
    ...
    writeln!(
        output,
        "    /// Sets the element at the given index by copying from the provided value."
    )
    .unwrap();
    writeln!(
        output,
        "    /// IMPORTANT: Elements must be set in order (0, 1, 2, ...) because offset"
    )
    .unwrap();
    writeln!(
        output,
        "    /// calculation uses the provided value's size."
    )
    .unwrap();
    writeln!(output, "    /// Panics if index is out of bounds.").unwrap();
    writeln!(
    ...
    writeln!(
        output,
        "        // Calculate base offset from header fields"
    )
    .unwrap();
    ...
    writeln!(
        output,
        "        // Calculate offset by walking through previously set elements"
    )
    .unwrap();
    writeln!(
        output,
        "        // Elements MUST be set in sequential order (0, 1, 2, ...)"
    )
    .unwrap();
    ...
    writeln!(
        output,
        "            // Read element size from what was already written"
    )
    .unwrap();
    writeln!(
        output,
        "            let elem = {} {{ data: &self.data[off..] }};",
        elem_type_name
    )
    .unwrap();
    writeln!(output, "            off += elem.size();").unwrap();
    ...
    writeln!(
        output,
        "        self.data[off..off + value_size].copy_from_slice(&value.data[..value_size]);"
    )
    .unwrap();
    writeln!(output, "    }}\n").unwrap();
}
