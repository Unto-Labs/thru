...
1. **CLI front-end** (`rpc/abi/abi_gen/src/main.rs`, `cmds/mod.rs`)
...
2. **Import resolution** (`rpc/abi/abi_loader/src/`)
...
3. **Dependency analysis & validation** (`rpc/abi/abi_gen/src/dependency.rs`)
...
4. **Type resolution** (`rpc/abi/abi_gen/src/abi/resolved.rs`)
   ...
   - Enforces forward-reference bans, ensures enums with heterogenous payload sizes surface `Size::Variable`, and wires in size-discriminated union invariants per `rpc/abi/enum-fams.md`.
5. **Layout graph + IR build** (`rpc/abi/abi_gen/src/abi/layout_graph.rs`, `codegen/shared/builder.rs`)
...
- When touching backends, prefer regenerating a single compliance case (`rpc/abi/abi_gen/scripts/run_ir_parity_checks.py --case <name>`) so you can diff the generated artifacts quickly.
