...
- **address**: A Thru address (46-character `ta`-prefixed encoding). `.thru` / TNS names are not supported yet for ABI imports.
- **target**:
...
3. Validating remaining on-chain imports use the same network and failing if they do not
