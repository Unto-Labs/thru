...
- Tests/Automation:
  - Update `rpc/abi/abi_gen/scripts/run_ir_parity_checks.py` to include Rust parity-by-default.
...
- Phase 5 (compliance harness): enable TS parity mode in `compliance_harness_rust`; run full suite via `cargo test -p compliance_harness_rust` and `python3 rpc/abi/abi_gen/scripts/run_ir_parity_checks.py --run-harness --language ts,rust`; gate CI on both.
