...
use thru_base::{
    tn_signature::verify_transaction,
    txn_lib::{TN_TXN_SIGNATURE_SZ, WireTxnHdrV1},
};
...
fn main() {
    ...
    if bytes.len() < min_size {
        eprintln!(
            "transaction too short: {} bytes (min {})",
            bytes.len(),
            min_size
        );
        process::exit(1);
    ...
    let sig: &[u8; TN_TXN_SIGNATURE_SZ] = bytes[bytes.len() - TN_TXN_SIGNATURE_SZ..]
        .try_into()
        .unwrap();
