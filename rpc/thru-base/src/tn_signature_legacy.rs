...
//!
//! This is the domain-separated scheme used **before** commit
//! `31c178baa` ("standardize Ed25519 signing to RFC-8032 with per-domain DST"):
//! the domain is mixed *inside* the hash as a 128-byte block (an 8-byte
//! big-endian tag padded with zeros), i.e.
//!   r = H(domain_block ‖ prefix ‖ msg)
//!   k = H(domain_block ‖ R ‖ A ‖ msg)
//!   S = (r + k·scalar) mod L
//! rather than signing `M = DST ‖ context` with a stock Ed25519 signer.
//!
//! It is preserved verbatim (own [`SignatureDomain`], [`TnSignatureError`], and
//! `domain_block`) so a downstream fork on its own upgrade path can keep the old
//! wire behavior. It is intentionally NOT the current scheme — new code must use
//! [`crate::tn_signature`]. Signatures produced here are NOT interchangeable with
//! [`crate::tn_signature`].
