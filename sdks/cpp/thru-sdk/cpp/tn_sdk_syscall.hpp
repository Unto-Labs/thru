...
constexpr ulong TSDK_INVOKE_AUTH_MAGIC = 0xC3F7A1D9E5B20846UL;

/* Invoke auth validation/revert error codes */
constexpr ulong TSDK_INVOKE_AUTH_ERR_BAD_MAGIC             = 0xBAD0A170UL;
constexpr ulong TSDK_INVOKE_AUTH_ERR_UNOWNED_AUTH_ACCOUNT  = 0xBAD0A171UL;
constexpr ulong TSDK_INVOKE_AUTH_ERR_INVALID_ACCOUNT_INDEX = 0xBAD0A173UL;
...
struct tsdk_invoke_auth {
  ...
  /* Account indices are laid out immediately after this header:
     [auth_cnt entries][deauth_cnt entries] */
  ushort const * auth_idxs( void ) const {
    ulong const acc_idxs_off = sizeof( ulong ) + sizeof( ushort ) + sizeof( ushort );
    return reinterpret_cast<ushort const *>( reinterpret_cast<uchar const *>( this ) + acc_idxs_off );
  }

  ushort const * deauth_idxs( void ) const {
    return auth_idxs( ) + auth_cnt;
  }
};
...
inline ulong
account_create( ulong account_idx, std::span<const std::byte, TN_SEED_SIZE> seed,
...
inline ulong
account_create_ephemeral( ulong account_idx,
...
inline ulong
account_compress( ulong account_idx,
                  std::span<const std::byte> proof ) {
  return tsys_account_compress( account_idx, proof.data( ), proof.size( ));
}

inline ulong
account_decompress( ulong account_idx,
...
inline ulong
invoke( std::span<const std::byte> instr_data,
        ushort program_account_idx, ulong& invoke_error,
        tsdk_invoke_auth_t const * auth = nullptr ) {
  return tsys_invoke( instr_data.data( ), instr_data.size( ), program_account_idx, auth, &invoke_error );
}

inline ulong
log( std::span<const std::byte> data ) {
  return tsys_log( data.data( ), data.size( ));
}

inline ulong
emit_event( std::span<const std::byte> data ) {
  return tsys_emit_event( data.data( ), data.size( ));
}

inline ulong
set_account_data_writable( ulong account_idx ) {
  return tsys_set_account_data_writable( account_idx );
}

inline ulong
account_transfer( ulong from_account_idx, ulong to_account_idx,
                  ulong amount ) {
  return tsys_account_transfer( from_account_idx, to_account_idx, amount );
}

[[noreturn]] inline void
exit( ulong exit_code, bool revert = false ) {
  tsys_exit( exit_code, revert ? 1UL : 0UL );
}

inline ulong
account_set_flags( ushort account_idx, uchar flags ) {
  return tsys_account_set_flags( account_idx, flags );
}

inline ulong
account_create_eoa( ulong                      account_idx,
                    const signature_t *        signature,
                    std::span<const std::byte> proof ) {
