/* The implementation below was derived from Firedancer (Apache-2.0)
   Source:
     https://github.com/firedancer-io/firedancer/blob/main/src/util/tmpl/fd_map_dynamic.c
   */
...
/* Declare ultra high performance dynamic key-val maps of bounded
   run time size.  Typical usage:
     ...
     struct mymap {
       ...
       uint  hash; // Technically "MAP_HASH_T MAP_HASH;" (default is uint hash),
  ==mymap_hash(key)
     ...
     #include "util/tmpl/tsdk_map_dynamic.c"
  ...
  library in the compilation unit:
    ...
    mymap_t * mymap_join     ( void *    shmap                  ); // Indexed
  [0,2^lg_slot_cnt) void *    mymap_leave    ( mymap_t * map ); void *
  mymap_delete   ( void *    shmap                  );
  ...
  entry from the map is assign the entry's key to MAP_KEY_NULL.
    ...
    if( TSDK_UNLIKELY( !slot ) ) ... handle failure (cxx_key was not moved)
    else {
...
#else
#if !defined(MAP_KEY_NULL) || !defined(MAP_KEY_INVAL) ||                       \
    !defined(MAP_KEY_EQUAL) || !defined(MAP_KEY_EQUAL_IS_SLOW) ||              \
    !defined(MAP_KEY_HASH)
#error                                                                         \
    "Define MAP_KEY_NULL, MAP_KEY_INVAL, MAP_KEY_EQUAL, MAP_KEY_EQUAL_IS_SLOW, and MAP_KEY_HASH if using a custom MAP_KEY_T"
#endif
#endif
...
#ifndef MAP_KEY_HASH
#define MAP_KEY_HASH(key) ((MAP_HASH_T)tsdk_ulong_hash(key))
#endif
...
#if TSDK_TMPL_USE_HANDHOLDING
#include "../log/tsdk_log.h"
#endif

/* Implementation *****************************************************/

#define MAP_(n) TSDK_EXPAND_THEN_CONCAT3(MAP_NAME, _, n)
...
TSDK_FN_CONST static inline MAP_(private_t) *
...
TSDK_FN_CONST static inline MAP_(private_t) const* MAP_(
    private_from_slot_const)(MAP_T const* slot) {
  ulong slot_ofs = offsetof(MAP_(private_t), slot);
...
TSDK_FN_CONST static inline ulong MAP_(private_start)(MAP_HASH_T hash,
                                                      ulong slot_mask) {
  return (ulong)(hash & (MAP_HASH_T)slot_mask);
}
TSDK_FN_CONST static inline ulong MAP_(private_next)(ulong slot,
                                                     ulong slot_mask) {
...
TSDK_FN_CONST static inline ulong MAP_(align)(void) {
  return alignof(MAP_(private_t));
}

TSDK_FN_CONST static inline ulong MAP_(footprint)(int lg_slot_cnt) {
  ulong slot_cnt = 1UL << lg_slot_cnt;
  return tsdk_ulong_align_up(tsdk_ulong_align_up(24UL, alignof(MAP_T)) +
                                 sizeof(MAP_T) * slot_cnt,
                             alignof(MAP_(private_t)));
}

static inline void* MAP_(new)(void* shmem, int lg_slot_cnt) {
#if TSDK_TMPL_USE_HANDHOLDING
  if (TSDK_UNLIKELY(!tsdk_ulong_is_aligned((ulong)shmem, MAP_(align)())))
    TSDK_LOG_CRIT(("unaligned shmem"));
  if (TSDK_UNLIKELY((lg_slot_cnt < 0) | (lg_slot_cnt > 63)))
    TSDK_LOG_CRIT(("invalid lg_slot_cnt"));
#endif
  ulong slot_cnt = 1UL << lg_slot_cnt;
...
TSDK_FN_PURE static inline ulong MAP_(key_cnt)(MAP_T const* slot) {
  return MAP_(private_from_slot_const)(slot)->key_cnt;
}
TSDK_FN_PURE static inline ulong MAP_(key_max)(MAP_T const* slot) {
  return MAP_(private_from_slot_const)(slot)->slot_mask;
}
TSDK_FN_PURE static inline int MAP_(lg_slot_cnt)(MAP_T const* slot) {
  return MAP_(private_from_slot_const)(slot)->lg_slot_cnt;
}
TSDK_FN_PURE static inline ulong MAP_(slot_cnt)(MAP_T const* slot) {
  return MAP_(private_from_slot_const)(slot)->slot_mask + 1UL;
}

TSDK_FN_CONST static inline ulong MAP_(slot_idx)(MAP_T const* map,
                                                 MAP_T const* entry) {
#if TSDK_TMPL_USE_HANDHOLDING
  if (TSDK_UNLIKELY(((ulong)(entry - map) >= MAP_(slot_cnt)(map)) |
                    (map > entry)))
    TSDK_LOG_CRIT(("index out of bounds"));
#endif
  return (ulong)(entry - map);
}

TSDK_FN_CONST static inline MAP_KEY_T MAP_(key_null)(void) {
  return (MAP_KEY_NULL);
}

/* These are TSDK_FN_PURE instead of TSDK_FN_CONST in case a non-POD
   MAP_KEY_T.  FIXME: CONSIDER LETTING THE COMPILER SORT THIS OUT? */

TSDK_FN_PURE static inline int MAP_(key_inval)(MAP_KEY_T k0) {
  return (MAP_KEY_INVAL(k0));
}
TSDK_FN_PURE static inline int MAP_(key_equal)(MAP_KEY_T k0, MAP_KEY_T k1) {
  return (MAP_KEY_EQUAL(k0, k1));
}

TSDK_FN_PURE static inline MAP_HASH_T MAP_(key_hash)(MAP_KEY_T key) {
  return (MAP_KEY_HASH(key));
}

TSDK_FN_UNUSED static MAP_T* /* Work around -Winline */
MAP_(insert)(MAP_T* map, MAP_KEY_T key) {
#if TSDK_TMPL_USE_HANDHOLDING
  if (TSDK_UNLIKELY(MAP_(key_inval)(key)))
    TSDK_LOG_CRIT(("invalid key"));
#endif
  MAP_(private_t)* hdr = MAP_(private_from_slot)(map);

  ulong key_cnt = hdr->key_cnt;
  ulong slot_mask = hdr->slot_mask; /* == key_max (FIXME: MAKE KEY_MAX DISTINCT
                                       FROM SLOT_MASK?) */
  if (TSDK_UNLIKELY(key_cnt >= slot_mask))
    return NULL;
  ...
  for (;;) {
    ...
    if (TSDK_LIKELY(MAP_(key_inval)(map_key)))
      break;                             /* Optimize for not found */
#if MAP_MEMOIZE && MAP_KEY_EQUAL_IS_SLOW /* ... and then for searching */
    if (TSDK_UNLIKELY(m->MAP_HASH == hash && MAP_(key_equal)(map_key, key)))
      return NULL;
#else
    if (TSDK_UNLIKELY(MAP_(key_equal)(map_key, key)))
      return NULL;
#endif
...
static inline void MAP_(remove)(MAP_T* map, MAP_T* entry) {
  MAP_(private_t)* hdr = MAP_(private_from_slot)(map);
...
static inline void MAP_(clear)(MAP_T* map) {
  MAP_(private_t)* hdr = MAP_(private_from_slot)(map);
  hdr->key_cnt = 0UL;
...
TSDK_FN_PURE TSDK_FN_UNUSED static MAP_T* /* Work around -Winline */
MAP_(query)(MAP_T* map, MAP_KEY_T key, MAP_T* null) {
#if TSDK_TMPL_USE_HANDHOLDING
  if (TSDK_UNLIKELY(MAP_KEY_INVAL(key)))
    TSDK_LOG_CRIT(("invalid key"));
#endif
...
#define MAP_IMPL_QUERY_FOUND                                                   \
  (hash == m->MAP_HASH && MAP_(key_equal)(map_key, key))
#else
#define MAP_IMPL_QUERY_FOUND MAP_(key_equal)(map_key, key)
    ...
    if (empty)
      m = null; /* cmov */
    TSDK_COMPILER_FORGET(done);
    if (TSDK_LIKELY(done))
      break;
#elif MAP_QUERY_OPT == 1
    if (TSDK_LIKELY(MAP_IMPL_QUERY_FOUND))
      break;
    if (TSDK_LIKELY(MAP_(key_inval)(map_key)))
      return null;
#else
