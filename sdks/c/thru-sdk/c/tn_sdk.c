...
void *
memset( void * dest, int c, ulong n ) {
  uchar * p = (uchar *)dest;
  while( n-- )
    *p++ = (uchar)c;
  return dest;
...
void *
memcpy( void * dest, void const * src, ulong n ) {
  uchar *       d = (uchar *)dest;
...
int
memcmp( void const * s1, void const * s2, ulong n ) {
...
tsdk_block_ctx_t const *
tsdk_get_current_block_ctx( void ) {
  return (tsdk_block_ctx_t const *)TSDK_ADDR( TSDK_SEG_TYPE_READONLY_DATA,
                                          TSDK_SEG_IDX_BLOCK_CTX, 0UL );
}

tsdk_block_ctx_t const *
tsdk_get_past_block_ctx( ulong blocks_in_past ) {
...
int
tsdk_is_account_authorized_by_idx( ushort account_idx ) {
  /* Fee payer is always authorized */
  if( account_idx == 0 ) return 1;

  tsdk_shadow_stack_t const * ss = tsdk_get_shadow_stack( );

  /* Current program is always authorized */
  ushort current_prog = ss->stack_frames[ ss->call_depth ].program_acc_idx;
  if( current_prog == account_idx ) return 1;

  /* No parent frames to check */
  if( ss->call_depth == 1 ) return 0;

  tn_pubkey_t const *         accs        = tsdk_txn_get_acct_addrs( tsdk_get_txn( ) );
  tsdk_account_meta_t const * target_meta = tsdk_get_account_meta( account_idx );

  /* Walk shadow stack from most recent parent down to root.
     First match wins — deeper frames override shallower ones. */
  for( ushort i = (ushort)( ss->call_depth - 1U ); i > 0; i-- ) {
    tsdk_shadow_stack_frame_t const * frame = &ss->stack_frames[ i ];
    ulong auth_ptr = frame->saved_regs[ 13 ]; /* a3 register */

    if( auth_ptr != 0UL ) {
      tsdk_invoke_auth_t const * auth = (tsdk_invoke_auth_t const *)auth_ptr;
      if( auth->magic == TSDK_INVOKE_AUTH_MAGIC ) {
        /* Check deauth list (comes after auth entries) */
        ushort const * deauth = &auth->acc_idxs[ auth->auth_cnt ];
        for( ushort j = 0; j < auth->deauth_cnt; j++ ) {
          if( deauth[ j ] == account_idx ) return 0;
        }
        /* Check auth list */
        for( ushort j = 0; j < auth->auth_cnt; j++ ) {
          if( auth->acc_idxs[ j ] == account_idx ) {
            /* Frame's program must own the account */
            if( memcmp( &target_meta->owner, &accs[ frame->program_acc_idx ],
    ...
    if( frame->program_acc_idx == account_idx ) return 1;
  }
...
int
tsdk_is_account_authorized_by_pubkey( tn_pubkey_t const * pubkey ) {
  tn_pubkey_t const * accs = tsdk_txn_get_acct_addrs( tsdk_get_txn( ) );

  /* Fee payer is always authorized */
  if( memcmp( pubkey, &accs[ 0 ], sizeof( tn_pubkey_t ) ) == 0 ) return 1;

  tsdk_shadow_stack_t const * ss = tsdk_get_shadow_stack( );

  /* Current program is always authorized */
  ushort current_prog = ss->stack_frames[ ss->call_depth ].program_acc_idx;
  if( memcmp( pubkey, &accs[ current_prog ], sizeof( tn_pubkey_t ) ) == 0 ) return 1;
  ...
  for( ushort i = (ushort)( ss->call_depth - 1U ); i > 0; i-- ) {
    ...
    if( auth_ptr != 0UL ) {
      ...
      if( auth->magic == TSDK_INVOKE_AUTH_MAGIC ) {
        ...
        ushort const * deauth = &auth->acc_idxs[ auth->auth_cnt ];
        for( ushort j = 0; j < auth->deauth_cnt; j++ ) {
          if( memcmp( pubkey, &accs[ deauth[ j ] ], sizeof( tn_pubkey_t ) ) == 0 ) return 0;
        }
        /* Check auth list */
        for( ushort j = 0; j < auth->auth_cnt; j++ ) {
          if( memcmp( pubkey, &accs[ auth->acc_idxs[ j ] ], sizeof( tn_pubkey_t ) ) == 0 ) {
            /* Frame's program must own the account */
            tsdk_account_meta_t const * target_meta =
              tsdk_get_account_meta( auth->acc_idxs[ j ] );
            if( memcmp( &target_meta->owner, &accs[ frame->program_acc_idx ],
                        sizeof( tn_pubkey_t ) ) == 0 ) {
              return 1;
            }
    ...
    /* Existing behavior: program in call chain is authorized */
    if( memcmp( pubkey, &accs[ frame->program_acc_idx ],
                sizeof( tn_pubkey_t ) ) == 0 ) return 1;
  }
...
ushort
tsdk_get_current_program_acc_idx( void ) {
...
int
tsdk_is_account_owned_by_current_program( ushort account_idx ) {
...
int
tsdk_is_program_reentrant( void ) {
  tsdk_shadow_stack_t const * shadow_stack = tsdk_get_shadow_stack( );
...
int
tsdk_account_exists( ushort account_idx ) {
  tsdk_account_meta_t const * account_meta = tsdk_get_account_meta( account_idx );
  return account_meta->version != 0;
}
