...
#define TSDK_INVOKE_AUTH_ERR_BAD_MAGIC             (0xBAD0A170UL)
#define TSDK_INVOKE_AUTH_ERR_UNOWNED_AUTH_ACCOUNT  (0xBAD0A171UL)
#define TSDK_INVOKE_AUTH_ERR_INVALID_ACCOUNT_INDEX (0xBAD0A173UL)
...
ulong
tsys_account_create( ulong account_idx, uchar const seed[TN_SEED_SIZE],
...
ulong
tsys_account_create_ephemeral( ulong account_idx, uchar const seed[TN_SEED_SIZE] ) {
  ulong seed_val0, seed_val1, seed_val2, seed_val3;
  memcpy( &seed_val0, &seed[0], sizeof( ulong ));
  memcpy( &seed_val1, &seed[8], sizeof( ulong ));
  memcpy( &seed_val2, &seed[16], sizeof( ulong ));
...
ulong
tsys_account_compress( ulong account_idx, void const * proof, ulong proof_sz ) {
  register ulong a0 __asm__ ( "a0" ) = account_idx;
...
ulong
tsys_account_decompress( ulong account_idx, void const * meta,
...
tsys_invoke( void const * instr_data, ulong instr_data_sz,
  ...
  if( auth != (tsdk_invoke_auth_t const *)0 ) {
    ...
    ushort const * deauth_idxs = &auth->acc_idxs[ auth->auth_cnt ];
  ...
  register ulong a3 __asm__ ( "a3" ) = (ulong)auth;
...
ulong
tsys_log( void const * data, ulong data_sz ) {
...
ulong
tsys_emit_event( void const * data, ulong data_sz ) {
  register ulong a0 __asm__ ( "a0" ) = (ulong)data;
  register ulong a1 __asm__ ( "a1" ) = data_sz;
...
ulong
tsys_account_create_eoa( ulong account_idx, tn_signature_t const * signature,
                         void const * proof, ulong proof_sz ) {
  register ulong a0 __asm__ ( "a0" ) = account_idx;
