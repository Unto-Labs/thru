...
check_fedora_pkgs () {
  local REQUIRED_RPMS=( curl perl autoconf gettext-devel automake flex bison cmake gmp-devel protobuf-compiler lcov systemd-devel pkgconf patch python3 libmpc-devel mpfr-devel gawk texinfo patchutils zlib-devel expat-devel libslirp-devel meson ninja-build zstd zlib )
...
check_macos_pkgs () {
  local REQUIRED_FORMULAE=( curl perl autoconf gettext automake flex bison protobuf python@3.12 gawk gnu-sed make gmp mpfr libmpc isl zlib expat texinfo flock libslirp meson ninja zstd llvm@16 )
...
install_riscv () {
  ...
  # Update PATH for macOS to include LLVM tools
  if [[ "$OS" == "Darwin" ]]; then
    # Add LLVM@16 to PATH for macOS builds
    if [[ -d "/opt/homebrew/opt/llvm@16/bin" ]]; then
      export PATH="/opt/homebrew/opt/llvm@16/bin:$PATH"
      echo "[+] Added /opt/homebrew/opt/llvm@16/bin to PATH"
    elif [[ -d "/usr/local/opt/llvm@16/bin" ]]; then
      export PATH="/usr/local/opt/llvm@16/bin:$PATH"
      echo "[+] Added /usr/local/opt/llvm@16/bin to PATH"
    else
      echo "[!] Warning: LLVM@16 not found in expected locations"
    fi
    export CC=clang
    export CXX=clang++
  fi
